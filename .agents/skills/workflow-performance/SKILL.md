...
**IMPORTANT MANDATORY Steps:** $scout -> $investigate -> $plan-hard -> $why-review -> $plan-review -> $why-review -> $plan-validate -> $why-review -> $code -> $test -> $workflow-review-changes -> $sre-review -> $docs-update -> $watzup -> $workflow-end
...
**Steps:** $scout → $investigate → $plan-hard → $why-review → $plan-review → $why-review → $plan-validate → $why-review → $code → $test → $workflow-review-changes → $sre-review → $docs-update → $watzup → $workflow-end
...
**Goal:** [Workflow] Trigger Performance Optimization workflow — investigate bottlenecks, optimize queries, reduce latency.
