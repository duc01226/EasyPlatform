---
name: fix-issue
description: '[Implementation] Use when you need to debug and fix GitHub issues with systematic investigation.'
---
...
- Verify each hypothesis against an actual code trace before acting — do NOT assume the first hypothesis is correct — why: the first guess is usually the nearest-attention trap, not the cause
...
> **⚠️ Validate Before Fix (NON-NEGOTIABLE):** After root cause analysis, MUST ATTENTION present findings + proposed fix to user via a direct user question and get explicit approval BEFORE any code changes. No silent fixes.
...
**🛑 Present root cause + proposed fix → a direct user question → wait for user approval before implementing.**
