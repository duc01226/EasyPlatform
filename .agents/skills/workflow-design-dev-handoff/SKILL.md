...
**IMPORTANT MANDATORY Steps:** $design-spec -> $review-artifact -> $handoff -> $plan-hard -> $plan-review -> $plan-validate -> $workflow-end
...
**Steps:** $design-spec → $review-artifact → $handoff → $plan-hard → $plan-review → $plan-validate → $workflow-end
...
**Goal:** [Workflow] Trigger Designer to Dev Handoff workflow — designer hands off design spec to developer for implementation.
