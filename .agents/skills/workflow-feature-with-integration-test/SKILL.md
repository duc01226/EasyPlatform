...
description: '[Workflow] Use when activating the Feature with Integration Test workflow for spec-first feature implementation.'
...
**Goal:** [Workflow] Trigger Feature with Integration Test workflow — implement a well-defined feature with spec-first integration testing: spec writing before implementation, plan refinement, and test verification.
...
| `$plan-hard`   | 2nd (pos 9)  | PLAN₂: Implementation + integration test plan (after TDD specs) |
...
**IMPORTANT MANDATORY Steps:** $scout -> $feature-investigation -> $domain-analysis (if entity changes) -> $plan-hard -> $plan-review -> $plan-validate -> $why-review -> $tdd-spec -> $tdd-spec-review -> $plan-hard -> $plan-review -> $cook -> $review-domain-entities (if entity changes) -> $integration-test -> $integration-test-review -> $test -> $workflow-review-changes -> $sre-review -> $security -> $changelog -> $test -> $docs-update -> $watzup -> $workflow-end
...
**Steps:** $scout → $feature-investigation → $domain-analysis (if entity changes) → $plan-hard → $plan-review → $plan-validate → $why-review → $tdd-spec → $tdd-spec-review → $plan-hard → $plan-review → $cook → $review-domain-entities (if entity changes) → $integration-test → $integration-test-review → $test → $workflow-review-changes → $sre-review → $security → $changelog → $test → $docs-update → $watzup → $workflow-end
