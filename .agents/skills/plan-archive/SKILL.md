...
Use a direct user question tool to ask if user wants to document journal entries or not.
Skip this step if user selects "No".
If user selects "Yes":

- Analyze the information in previous steps.
- Use Task tool with `agent_type="journal-writer"` in parallel to document all plans.
- Journal entries should be concise and focused on the most important events, key changes, impacts, and decisions.
- Keep journal entries in the `./docs/journals/` directory.

### Step 3: Ask user to confirm the action before archiving these plans

Use a direct user question tool to ask if user wants to proceed with archiving these plans, select specific plans to archive or all completed plans only.
Use a direct user question tool to ask if user wants to delete permanently or move to the `./plans/archive` directory.
...
Use a direct user question tool to ask if user wants to commit the changes with these options:
