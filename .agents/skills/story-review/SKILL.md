...
After completing Round 1 checklist evaluation, spawn a **fresh `general-purpose` sub-agent** for Round 2 using the canonical Agent template from `SYNC:review-protocol-injection` above. Story artifact reviews are NOT code reviews — use `agent_type: "general-purpose"`. When constructing the Agent call prompt:
...
- **"$plan-hard (Recommended)"** — Create implementation plan from validated stories
