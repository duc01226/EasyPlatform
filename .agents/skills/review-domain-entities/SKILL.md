...
> - Max 3 fresh-subagent rounds per review — escalate via a direct user question if still FAIL
...
>
> **How:** Spawn NEW `spawn_agent` with `code-reviewer` subagent_type. Sub-agent re-reads ALL files from scratch via its own tool calls — never pass file contents inline. Sub-agent writes report to `plans/reports/domain-entities-round{N}-{date}.md`. Main agent reads report, integrates findings without filtering.
...
4. Max 3 fresh rounds → escalate via a direct user question if still failing
