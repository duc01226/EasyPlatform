...
1. Glob `plans/*$plan-hard.md` sorted by modification time — find active plan directory
2. Read `plan.md` — project scope, goals, prior decisions
3. Read all `{plan-dir}$research/*.md` — avoid duplicating prior work
4. Read `docs/project-reference/domain-entities-reference.md` (if exists) — project's single source of truth for domain entities (Codex has no hook injection — open this file directly before proceeding)
...
**MUST ATTENTION update `{plan-dir}$plan-hard.md`** with domain model summary section after completing analysis.

### Step 1: Load Business Context

Read artifacts from prior workflow steps (search `plans/` + `team-artifacts/`):

- Active plan (`{plan-dir}$plan-hard.md`) — scope, goals, constraints
...
Read `{plan-dir}$plan-hard.md`, append/update `## Domain Model` section:
...
{plan-dir}$plan-hard.md                              # Updated with domain model summary
...
- **"$plan-hard"** — If tech stack already decided and entity quality already reviewed
