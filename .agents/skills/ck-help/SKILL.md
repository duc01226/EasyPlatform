...
- `$plan-hard` then `$code` is the correct flow; NEVER suggest `$plan-hard` then `$cook`
...
- **`$plan-hard` → `$code`**: Plan first, then execute the plan
- **`$cook`**: Standalone - plans internally, no separate `$plan-hard` needed
- **NEVER** suggest `$plan-hard` → `$cook` (cook has its own planning)
