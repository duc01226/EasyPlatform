...
1. **Search for active plan** — Glob `plans/*$plan-hard.md` sorted by modification time, or check the current task list for plan context
...
- **Every story set MUST ATTENTION include a Story Dependencies table** — with types: `must-after`, `can-parallel`, `blocked-by`, `independent`. This enables `$prioritize` and `$plan-hard` to respect implementation ordering.
