...
```
1. Explicit slash command invocation — message STARTS with /command-name as first token (e.g. `$plan-hard do X`). Skill names referenced as nouns (e.g. "update /skill-a") are NOT slash commands — workflow detection required.
...
4. FALLBACK -> MUST ATTENTION invoke $plan-hard <prompt> FIRST
```
...
1. **MANDATORY task creation for file-modifying prompts** — If the prompt could result in ANY file changes, you MUST ATTENTION create task tracking items BEFORE making changes.
