...
// CONSEQUENCE: Creates deployment coupling (cannot independently deploy services),
// hidden data contracts (schema changes break without compiler errors),
// and migration cascades across service boundaries.
...
// CONSEQUENCE: Custom interfaces bypass platform audit trails, retry logic,
// and unit-of-work management. Extensions on IPlatformQueryableRootRepository
// get these automatically and remain swappable across DB engines.
...
// CONSEQUENCE: Throwing exceptions for validation bypasses fluent chain aggregation,
// loses structured error context (field + message), and prevents collecting all
// validation failures in one pass (first-throw-wins vs collect-all).
...
// CONSEQUENCE: Side effects in handlers create untestable coupling (must mock
// notification service for every command test), break when handler execution order
// changes, and cannot be independently retried on failure.
...
// CONSEQUENCE: Handler mapping creates coupling between transport and domain layers,
// making neither independently testable. When DTO shape changes, every handler that
// maps it must be updated instead of one MapToEntity() method.
```
...
// CONSEQUENCE: Bypasses centralized error handling, auth token injection,
// response caching, and request deduplication provided by PlatformApiService base.
...
// CONSEQUENCE: Manual signals lose loading/error state tracking, subscription
// lifecycle management, caching, and devMode validation that PlatformVmStore provides.
// Each component reinvents the same boilerplate differently.

// ❌ DON'T: Missing subscription cleanup
this.data$.subscribe(...);
// ✅ DO: Always use untilDestroyed
this.data$.pipe(this.untilDestroyed()).subscribe(...);
// CONSEQUENCE: Memory leak on every component destroy. Subscriptions survive
// component lifecycle, causing stale callbacks, phantom state updates,
// and progressively degrading app performance.

// ❌ DON'T: Assume method names
this.someMethod();
// ✅ DO: Check base class APIs first through IntelliSense
// CONSEQUENCE: Platform base classes provide many utility methods (loading state,
// error handling, trackBy helpers). Reimplementing them creates inconsistent
// behavior and misses framework improvements on upgrade.
```
