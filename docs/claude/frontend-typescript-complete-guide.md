...
> Angular 19 + Easy.Platform Framework | BravoSUITE Enterprise HR Platform
...
10. [BravoCommon Library](#10-bravocommon-library)
...
// Currency
{{ amount | bravoCurrency:'VND' }}
```

### Utility Classes

```typescript
import { BravoArrayUtil, BravoDateUtil, BravoStringUtil } from '@libs/bravo-common';

// Array utilities
const dict = BravoArrayUtil.toDictionary(items, x => x.id);
const grouped = BravoArrayUtil.groupBy(items, x => x.category);
const unique = BravoArrayUtil.distinctBy(items, x => x.email);

// Date utilities
const formatted = BravoDateUtil.format(new Date(), 'DD/MM/YYYY');
const diff = BravoDateUtil.diffInDays(date1, date2);
const isWeekend = BravoDateUtil.isWeekend(date);

// String utilities
const truncated = BravoStringUtil.truncate(text, 100);
const slug = BravoStringUtil.toSlug(title);
const isEmpty = BravoStringUtil.isNullOrEmpty(value);
```
...
│   ├── bravo-common/            # Shared UI components, directives, pipes
│   └── bravo-domain/            # Business domain (APIs, models)
├── apps/
│   ├── growth-for-company/      # bravoGROWTH app (Port 4206)
│   ├── employee-platform/       # Employee platform (Port 4205)
│   └── candidate-app/           # Candidate app (Port 4204)
└── docs/
...
npm run dev-start:candidate       # Port 4204

# Build
nx build growth-for-company
nx build bravo-domain
...
# Lint
nx lint bravo-domain
nx lint growth-for-company
```
