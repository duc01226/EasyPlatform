...
> Common issues and solutions for YourProject development and Claude Code
...
| **Missing repositories**       | Search for service-specific repository interfaces in Domain project    |
...
| **Validation not working**     | Check if using project validation fluent API correctly                 |
...
```bash
# Check project framework package versions
dotnet list package | grep {YourFrameworkPackage}

# Update framework packages
dotnet add package {YourFrameworkPackage} --version <latest>
```
...
```csharp
// Error: Cannot resolve IServiceARepository<Entity>

// Solution: Check registration in DI container
// Ensure entity is registered in DbContext
public class ServiceADbContext : PlatformDbContext
{
...
When debugging issues, follow the [AI Debugging Protocol](./AI-DEBUGGING-PROTOCOL.md):
...
_Source: YourProject troubleshooting guide | Claude Code configuration issues_
