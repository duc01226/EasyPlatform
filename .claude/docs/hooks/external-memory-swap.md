...
runHook('tool-output-swap', event => {
    ...
    const entry = externalize(sessionId || process.env.CK_SESSION_ID || 'default', toolName, toolInput, toolResult);
...
function extractSummary(content, toolName) {
    ...
    if (toolName === 'Grep') {
        ...
        return `${lines.length} matches. Preview: ${lines
            .slice(0, 3)
            .map(l => l.slice(0, 80))
            .join(' | ')}`;
    }
...
- [README.md](./README.md) - Execution order by event, state file reference
- [../configuration/README.md](../configuration/README.md) - Settings configuration

---

_Source: `.claude/hooks/lib/swap-engine.cjs` (418 lines) | Tests: 50 passing_
