...
Detailed patterns and examples for the test-ui skill.
...
# Save to file
node aria-snapshot.js --url https://example.com --output ./.claude/test-ui/snapshots/page.yaml
```
...
```bash
mkdir -p .claude/test-ui/snapshots
SESSION="$(date +%Y%m%d-%H%M%S)"
node aria-snapshot.js --url https://example.com --output .claude/test-ui/snapshots/$SESSION.yaml
```
...
Store in `<project>/.claude/test-ui/screenshots/`:

```bash
# Basic screenshot
node screenshot.js --url https://example.com --output ./.claude/test-ui/screenshots/page.png

# Full page
node screenshot.js --url https://example.com --output ./.claude/test-ui/screenshots/page.png --full-page true

# Specific element
node screenshot.js --url https://example.com --selector ".main-content" --output ./.claude/test-ui/screenshots/element.png
```
...
```bash
SESSION="$(date +%Y%m%d-%H%M%S)"
mkdir -p .claude/test-ui/logs/$SESSION

node console.js --url https://example.com --duration 10000 > .claude/test-ui/logs/$SESSION/console.json
node network.js --url https://example.com > .claude/test-ui/logs/$SESSION/network.json

# View errors
jq '.messages[] | select(.type=="error")' .claude/test-ui/logs/$SESSION/console.json
```
...
node screenshot.js --output ./.claude/skills/test-ui/screenshots/debug.png
...
Write scripts to `<project>/.claude/test-ui/tmp/`:

```bash
mkdir -p $SKILL_DIR/.claude/test-ui/tmp

cat > $SKILL_DIR/.claude/test-ui/tmp/login-test.js << 'EOF'
...
node $SKILL_DIR/.claude/test-ui/tmp/login-test.js
```
