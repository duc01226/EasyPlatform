...
Copilot reads `.github/agents/*.agent.md` (repo-level) and `~/.copilot/agents/` (user-level). Claude Code reads `.claude/agents/`.

**This repo today**: agents are Claude-first in `.claude/agents/` with a generated `.codex/agents/` mirror; no `.github/agents/` surface exists. If/when one is added it must be generator-produced from `.claude/agents/` — never hand-copied.
