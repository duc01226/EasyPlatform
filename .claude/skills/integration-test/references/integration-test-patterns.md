...
Canonical patterns for subcutaneous integration tests using Easy.Platform. All use real DI — no mocks.
...
> **Prerequisite:** Full backend system must be running before tests execute. Tests use real DB, RabbitMQ, and depend on running API services for cross-service data sync. Local: `Bravo-DevStarts/StartDocker/BRAVO-APIS-DOCKER-Start.cmd` → wait for all services healthy.
...
- `Assert.ThrowsAsync<PlatformValidationException>` for negative cases (requires `using Easy.Platform.Common.Validations.Exceptions`)
...
Cross-service tests verify message bus flows between two independently booted service modules. Uses `PlatformCrossServiceFixture` to compose fixtures.
...
public class UserSyncCrossServiceTests : CrossServiceTestBase
{
    ...
    {
        // Arrange
        var email = PlatformIntegrationTestHelper.UniqueEmail("sync-test");
...
- `CrossServiceFixture` extends `PlatformCrossServiceFixture` — boots both services in-process
...
public class {Service}IntegrationTestFixture
    : PlatformServiceIntegrationTestFixture<{Service}ApiAspNetCoreModule>
{
...
**Key points:**
- `PlatformServiceIntegrationTestFixture<T>` handles: DI setup, module initialization (same as production `InitializeAsync()`), data seeding, teardown
...
- `PlatformCrossServiceFixture` validates all types extend `PlatformServiceIntegrationTestFixture<T>` at construction time
...
public class {Service}IntegrationTestDataSeeder : PlatformIntegrationTestDataSeeder
{
...
**Key points:**
- **Layer 1 — `PlatformApplicationDataSeeder`**: Production-like data (admin user, orgs, departments). Runs during `module.InitializeAsync()` → registered in service module. Test project inherits this automatically.
- **Layer 2 — `PlatformIntegrationTestDataSeeder`**: Test-specific reference data (templates, request types, settings). Runs in `SeedDataAsync()` after module init.
...
public abstract class {Service}ServiceIntegrationTestBase
    : PlatformServiceIntegrationTestWithAssertions<{Service}ApiAspNetCoreModule>
{
    /// <summary>
    /// Wire up service-specific repository for AssertEntity* methods.
    /// </summary>
    protected override Easy.Platform.Domain.Repositories.IPlatformRepository<TEntity, string>
...
**Key points:**
- Extends `PlatformServiceIntegrationTestWithAssertions<T>` (not `PlatformServiceIntegrationTestBase<T>`) to get `AssertEntityMatchesAsync`, `AssertEntityDeletedAsync` etc.
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    ...
    <ProjectReference Include="..\..\Platform\Easy.Platform.AutomationTest\Easy.Platform.AutomationTest.csproj" />
...
### Single-Service Helpers (from `PlatformServiceIntegrationTestBase<T>`)
...
| `AssertValidationFailsAsync(action, msg?)`   | Assert command throws `IPlatformValidationException`   |

### Database Assertion Helpers (from `PlatformServiceIntegrationTestWithAssertions<T>`)
...
### Utility Helpers (from `PlatformIntegrationTestHelper`)

| Method                                                    | Purpose                                            |
| --------------------------------------------------------- | -------------------------------------------------- |
| `PlatformIntegrationTestHelper.UniqueName("prefix")`      | `"prefix_{8-char-guid}"` for unique entity names   |
| `PlatformIntegrationTestHelper.UniqueId()`                | 12-char unique ID                                  |
| `PlatformIntegrationTestHelper.UniqueEmail("prefix")`     | `"prefix_{8-char-guid}@test.local"`                |
| `PlatformIntegrationTestHelper.WaitUntilAsync(condition)` | Poll condition with timeout (eventual consistency) |
