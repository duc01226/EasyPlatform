...
version: 1.0.0
description: '[Implementation] Quickly bootstrap a new project automatically'
...
1. **Research & Planning** — 6 parallel researcher subagents (idea, tech stack, design), then UI/UX + planner sequentially
2. **Implementation** — Execute plan with main agent + UI/UX subagent for frontend
3. **Testing** — Real tests, tester subagent, fix until all pass
4. **Code Review** — Iterate with code-reviewer until clean
5. **Documentation** — README, PDR, code standards, architecture, roadmap
6. **Final Report** — Summary + auto-commit (no push)
7. **Onboarding** — Step-by-step project setup with user
