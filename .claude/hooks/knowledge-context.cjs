...
const { KNOWLEDGE_CONTEXT: DEDUP_MARKER, DEDUP_LINES } = require('./lib/dedup-constants.cjs');
...
const WORKSPACE_TYPES = [
    { key: 'research', pathRegex: /docs[\\/]knowledge[\\/]research/i, template: 'docs/templates/research-report-template.md', label: 'Research & Synthesis' },
    { key: 'courses', pathRegex: /docs[\\/]knowledge[\\/]courses/i, template: 'docs/templates/course-outline-template.md', label: 'Course Material' },
    {
        key: 'marketing',
        pathRegex: /docs[\\/]knowledge[\\/]strategy[\\/]marketing/i,
        template: 'docs/templates/marketing-strategy-template.md',
        label: 'Marketing Strategy'
    },
    {
        key: 'business',
        pathRegex: /docs[\\/]knowledge[\\/]strategy[\\/]business/i,
        template: 'docs/templates/business-evaluation-template.md',
...
function shouldInject(filePath, transcriptPath) {
    ...
    if (wasRecentlyInjected(transcriptPath, DEDUP_MARKER, DEDUP_LINES.KNOWLEDGE_CONTEXT)) return false;
    return true;
}
...
function buildInjection(filePath) {
    ...
    const lines = ['', DEDUP_MARKER, '', `**Workspace:** ${workspace?.label || 'Knowledge Work'}`, `**File:** ${fileName}`, ''];

    // Template guidance
    if (workspace?.template) {
        lines.push('### Template', '', `Use enforced template: **\`${workspace.template}\`**`, '');
    }
    ...
    lines.push(
        ...
        '',
        '**MUST READ** `.claude/skills/shared/web-research-protocol.md`',
        '',
        '1. Follow web-research-protocol for all factual claims',
