...
const SCAN_DISMISS_FLAG = path.join(PROJECT_DIR, '.claude', '.scan-stale-dismissed');
...
function main() {
    try {
        ...
        // Fast path: config already populated → check staleness gate + graph warning
        if (isConfigPopulated()) {
            handleStalenessGate(userPrompt);

            // Soft warning: suggest /graph-build if graph.db doesn't exist yet
            // Non-blocking (exit 0) — graph is optional but valuable
            const graphDbPath = path.join(PROJECT_DIR, '.code-graph', 'graph.db');
            if (!fs.existsSync(graphDbPath)) {
                console.log(
                    '[graph] No knowledge graph found. Run /graph-build to enable ' +
                        'graph-powered code intelligence (frontend↔backend tracing, ' +
                        'blast radius analysis, cross-service flow detection).'
                );
            }
