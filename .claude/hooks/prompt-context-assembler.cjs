...
const { injectLessons, injectLessonReminder } = require('./lib/prompt-injections.cjs');
...
function buildReminder({
    ...
    return [
        ...
        `If the user prompt could be complex or vague, you **MANDATORY IMPORTANT MUST** use the \`AskUserQuestion\` tool to confirm your understanding of the request and clarify the user's intent before executing any task. Restate what you understood, ask clarifying questions if ambiguous, and only proceed after the user confirms. During confirmation, always detect the nearest matching workflow from the Workflow Catalog and use \`AskUserQuestion\` to ask the user whether to activate the detected workflow (Recommended) or execute directly without workflow.`,
        ``,
...
async function main() {
    try {
        ...
        // Skipped when recently injected (dedup), but lessons/reminder always run
        // ═══════════════════════════════════════════════════════════════════════════
        ...
        if (!skipMainBlock) {
            ...
            // INJECT DEV RULES FILE CONTENT (if exists and not recently injected)
