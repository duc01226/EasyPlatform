...
function buildCodingPatternContext(agentType) {
  ...
  const backendDoc =
    projConfig.framework?.backendPatternsDoc ||
    "docs/project-reference/backend-patterns-reference.md";
  const frontendDoc =
    projConfig.framework?.frontendPatternsDoc ||
    "docs/project-reference/frontend-patterns-reference.md";

  const stylingDoc =
    projConfig.contextGroups?.find((g) => g.stylingDoc)?.stylingDoc || null;
  const designSystemDoc =
    projConfig.contextGroups?.find((g) => g.designSystemDoc)?.designSystemDoc ||
    null;
...
function buildPlanContext(resolved, reportsPath, plansPath, docsPath) {
  ...
  if (activePlan) planLine = `- Plan: ${activePlan}`;
    ...
    planLine = `- Plan: none | Suggested: ${suggestedPlan}`;
...
function buildLanguageSection(config) {
  ...
  return [
    ...
    ...(hasThinking
      ...
      : []),
    ...(responseLanguage
      ? [`- Response: Respond in ${responseLanguage} (natural, fluent).`]
      : []),
    ``,
  ];
}
...
async function main() {
  try {
    ...
    const lines = [
      ...
      `- MUST READ \`.claude/workflows/development-rules.md\` for full rules, code quality guidelines, and pre-commit checklist.`,
      `- Reports → ${reportsPath}`,
      `- YAGNI / KISS / DRY`,
