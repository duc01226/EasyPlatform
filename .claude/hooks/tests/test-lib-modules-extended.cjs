...
function logResult(name, passed, message = "") {
  ...
  console.log(
    `  ${icon} ${name}${message ? `: ${COLORS.dim}${message}${COLORS.reset}` : ""}`,
  );
...
try {
  ...
  console.error(
    `${COLORS.red}Failed to load project-config-schema.cjs: ${e.message}${COLORS.reset}`,
  );
  process.exit(1);
}
...
logResult(
  "exports getRequiredSections",
  typeof getRequiredSections === "function",
);
...
logResult(
  "valid regex returns null",
  validateRegex("src[\\\\/]Services", "test") === null,
);
logResult(
  "invalid regex returns error",
  validateRegex("[invalid", "test") !== null,
);
...
const VALID_CONFIG = {
  ...
  backendServices: {
    patterns: [
      { name: "Svc", pathRegex: "src[\\\\/]svc", description: "desc" },
    ],
  ...
  frontendApps: {
    patterns: [
      { name: "App", pathRegex: "src[\\\\/]app", description: "desc" },
    ],
...

{
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      logResult(
        "docs/project-config.json passes schema",
        result.valid,
...

{
  ...
  logResult(
    "does NOT report missing backendServices (deprecated)",
    !result.errors.some((e) => e.includes("backendServices")),
  );
  logResult(
    "does NOT report missing frontendApps (deprecated)",
    !result.errors.some((e) => e.includes("frontendApps")),
  );
  logResult(
  ...
  logResult(
    "does NOT report missing sharedNamespace (deprecated)",
    !result.errors.some((e) => e.includes("sharedNamespace")),
  );
}
...

{
  ...
  const output = formatResult(result);
  logResult(
    "formats failing result",
    output.includes("FAILED") && output.includes("test error"),
  );
...

{
  ...
  // backendServices, frontendApps, sharedNamespace are now deprecated (optional)
  logResult(
    "excludes backendServices (deprecated)",
    !sections.includes("backendServices"),
  );
  logResult(
    "excludes frontendApps (deprecated)",
    !sections.includes("frontendApps"),
  );
  logResult("includes framework", sections.includes("framework"));
  logResult(
    "excludes sharedNamespace (deprecated)",
    !sections.includes("sharedNamespace"),
  );
}
...

{
  ...
  const minV2 = {
    ...
    designSystem: {
      docsPath: "docs/project-reference/design-system",
      appMappings: [
        { name: "A", pathRegexes: ["src[\\\\/]"], docFile: "A.md" },
      ],
    },
  };
  ...
  logResult(
    "no deprecation warnings in v2-only",
    !r.warnings.some((w) => w.includes("DEPRECATED")),
  );
  ...
  logResult(
    "schemaVersion string rejected",
    validateConfig(badVersion).errors.some((e) =>
  ...
  const withModules = {
    ...minV2,
    modules: [
      { name: "svc", kind: "backend-service", pathRegex: "Services[\\\\/]svc" },
    ],
  };
  ...
  logResult(
    "modules[] missing kind rejected",
    !validateConfig(badModules).valid,
  );
  ...
  logResult(
    "custom section freeform accepted",
    validateConfig(withCustom).valid,
  );
  ...
  logResult(
    "v1 sections produce deprecation warnings",
    v1Result.warnings.filter((w) => w.includes("DEPRECATED")).length >= 2,
  );
...

{
  ...
  logResult(
    "getContextGroup returns Backend name",
    csGroup && csGroup.name.includes("Backend"),
  );
...

{
  ...
  if (fs.existsSync(configPath)) {
    ...
    logResult(
      "real config passes schema",
      result.valid,
      result.errors.join("; "),
    );
    logResult(
      "real config has schemaVersion",
      realConfig.schemaVersion !== undefined,
    );
    logResult("real config has modules[]", Array.isArray(realConfig.modules));
    logResult(
      "real config has contextGroups[]",
      Array.isArray(realConfig.contextGroups),
    );
    logResult(
      "real config has project section",
      realConfig.project !== undefined,
    );
    logResult(
      "real config has testing section",
      realConfig.testing !== undefined,
    );
  } else {
    logResult(
      "real config file exists",
      false,
      "docs/project-config.json not found",
    );
  }
}
...

{
  ...
  logResult(
    "shows appMappings.pathRegexes field",
    output.includes('"pathRegexes"'),
  );
  logResult("shows appMappings.docFile field", output.includes('"docFile"'));
  logResult(
    "shows referenceDocs.filename field",
...

{
  ...
  logResult(
    "isIgnoredDir: .claude is ignored",
    isIgnoredDir(".claude") === true,
  );
  logResult("isIgnoredDir: .git is ignored", isIgnoredDir(".git") === true);
  logResult(
    "isIgnoredDir: .github is ignored",
    isIgnoredDir(".github") === true,
  );
  logResult(
    "isIgnoredDir: .vscode is ignored",
    isIgnoredDir(".vscode") === true,
  );
  logResult("isIgnoredDir: .idea is ignored", isIgnoredDir(".idea") === true);
  logResult(
    "isIgnoredDir: .devcontainer is ignored",
    isIgnoredDir(".devcontainer") === true,
  );
  logResult("isIgnoredDir: .husky is ignored", isIgnoredDir(".husky") === true);
  logResult(
    "isIgnoredDir: .cursor is ignored",
    isIgnoredDir(".cursor") === true,
  );
  logResult(
    "isIgnoredDir: .windsurf is ignored",
    isIgnoredDir(".windsurf") === true,
  );
  logResult(
    "isIgnoredDir: .circleci is ignored",
    isIgnoredDir(".circleci") === true,
  );
  logResult(
    "isIgnoredDir: .docker is ignored",
    isIgnoredDir(".docker") === true,
  );
  logResult(
    "isIgnoredDir: node_modules is ignored",
    isIgnoredDir("node_modules") === true,
  );
  logResult("isIgnoredDir: src is NOT ignored", isIgnoredDir("src") === false);
  logResult(
    "isIgnoredDir: docs is NOT ignored",
    isIgnoredDir("docs") === false,
  );
  ...
  logResult(
    "hasProjectContent: empty dir returns false",
    hasProjectContent(emptyDir) === false,
  );

  // Dir with only .claude → false
  const claudeOnlyDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ck-test-claude-"),
  );
  fs.mkdirSync(path.join(claudeOnlyDir, ".claude"));
  logResult(
    "hasProjectContent: .claude only returns false",
    hasProjectContent(claudeOnlyDir) === false,
  );

  // Dir with only .git + .claude → false
  const gitClaudeDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ck-test-gitclaude-"),
  );
  fs.mkdirSync(path.join(gitClaudeDir, ".claude"));
  fs.mkdirSync(path.join(gitClaudeDir, ".git"));
  logResult(
    "hasProjectContent: .git + .claude returns false",
    hasProjectContent(gitClaudeDir) === false,
  );

  // Dir with many dot-prefixed + node_modules → still false (no real content)
  const toolOnlyDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ck-test-toolonly-"),
  );
  ...
  logResult(
    "hasProjectContent: multiple tool dirs only returns false",
    hasProjectContent(toolOnlyDir) === false,
  );
  ...
  logResult(
    "hasProjectContent: src/ returns true",
    hasProjectContent(srcDir) === true,
  );
  ...
  logResult(
    "hasProjectContent: docs/ returns true",
    hasProjectContent(docsDir) === true,
  );

  // Nonexistent dir → false
  logResult(
    "hasProjectContent: nonexistent dir returns false",
    hasProjectContent("/nonexistent/path/xyz") === false,
  );
  ...
  logResult(
    "isGreenfieldProject: empty dir is greenfield",
    isGreenfieldProject(emptyDir) === true,
  );

  // Dir with .claude only → true (still greenfield)
  logResult(
    "isGreenfieldProject: .claude only is greenfield",
    isGreenfieldProject(claudeOnlyDir) === true,
  );

  // Dir with src/ containing files → false (not greenfield)
  const srcWithFiles = fs.mkdtempSync(
    path.join(os.tmpdir(), "ck-test-srcfiles-"),
  );
  fs.mkdirSync(path.join(srcWithFiles, "src"));
  fs.writeFileSync(
    path.join(srcWithFiles, "src", "main.ts"),
    'console.log("hello");',
  );
  logResult(
    "isGreenfieldProject: src/ with files is NOT greenfield",
    isGreenfieldProject(srcWithFiles) === false,
  );
  ...
  logResult(
    "isGreenfieldProject: package.json present is NOT greenfield",
    isGreenfieldProject(pkgDir) === false,
  );
  ...
  logResult(
    "isGreenfieldProject: .sln present is NOT greenfield",
    isGreenfieldProject(slnDir) === false,
  );
  ...
  logResult(
    "isGreenfieldProject: README + .claude only is greenfield",
    isGreenfieldProject(partialDir) === true,
  );
  ...
  logResult(
    "isGreenfieldProject: app/ with files is NOT greenfield",
    isGreenfieldProject(appDir) === false,
  );
  ...
  logResult(
    "isGreenfieldProject: lib/ with files is NOT greenfield",
    isGreenfieldProject(libDir) === false,
  );

  // Dir with server/ containing files → false
  const serverDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ck-test-serverdir-"),
  );
  fs.mkdirSync(path.join(serverDir, "server"));
  fs.writeFileSync(path.join(serverDir, "server", "main.go"), "package main");
  logResult(
    "isGreenfieldProject: server/ with files is NOT greenfield",
    isGreenfieldProject(serverDir) === false,
  );

  // Dir with docs/ + plans/ + team-artifacts/ but no code → still greenfield
  const planningDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "ck-test-planning-"),
  );
  ...
  logResult(
    "isGreenfieldProject: docs+plans+team-artifacts (no code) is greenfield",
    isGreenfieldProject(planningDir) === true,
  );

  // CODE_DIRECTORIES contains expected entries
  const {
    CODE_DIRECTORIES,
  } = require("../../hooks/lib/session-init-helpers.cjs");
  ...
  logResult(
    "CODE_DIRECTORIES includes server",
    CODE_DIRECTORIES.includes("server"),
  );
  logResult(
    "CODE_DIRECTORIES includes packages",
    CODE_DIRECTORIES.includes("packages"),
  );

  // IGNORED_ROOT_DIRS contains non-dot exceptions (dot-prefixed dirs handled by pattern)
  logResult(
    "IGNORED_ROOT_DIRS includes node_modules",
    IGNORED_ROOT_DIRS.has("node_modules"),
  );
  logResult(
    "IGNORED_ROOT_DIRS does NOT include .claude (handled by dot-prefix)",
    !IGNORED_ROOT_DIRS.has(".claude"),
  );
  logResult(
    "IGNORED_ROOT_DIRS does NOT include .git (handled by dot-prefix)",
    !IGNORED_ROOT_DIRS.has(".git"),
  );

  // MANIFEST_FILES contains expected entries
  logResult(
    "MANIFEST_FILES includes package.json",
    MANIFEST_FILES.includes("package.json"),
  );
  logResult("MANIFEST_FILES includes *.sln", MANIFEST_FILES.includes("*.sln"));
  logResult(
    "MANIFEST_FILES includes go.mod",
    MANIFEST_FILES.includes("go.mod"),
  );
...
{
  const ckSchemaPath = path.join(
    __dirname,
    "..",
    "lib",
    "ck-config-schema.cjs",
  );
  const { validateCkConfig, formatCkValidationResult, CK_SCHEMA } = require(
    ckSchemaPath,
  );

  // Module exports
  logResult(
    "ck: exports validateCkConfig",
    typeof validateCkConfig === "function",
  );
  logResult(
    "ck: exports formatCkValidationResult",
    typeof formatCkValidationResult === "function",
  );
  logResult(
    "ck: exports CK_SCHEMA",
    typeof CK_SCHEMA === "object" && CK_SCHEMA !== null,
  );
  ...
  // Invalid enum
  {
    ...
    logResult(
      "ck: invalid enum rejected",
      !r.valid &&
        r.errors.some((e) => e.includes("invalid value") && e.includes("yes")),
    );
  }
  ...
  // Out-of-range number (too high)
  {
    ...
    logResult(
      "ck: codingLevel too high rejected",
      !r.valid && r.errors.some((e) => e.includes("exceeds maximum")),
    );
  }
  ...
  // Out-of-range number (too low)
  {
    ...
    logResult(
      "ck: codingLevel too low rejected",
      !r.valid && r.errors.some((e) => e.includes("below minimum")),
    );
  }
  ...
  // Wrong type for number field
  {
    ...
    logResult(
      "ck: wrong type for number rejected",
      !r.valid && r.errors.some((e) => e.includes("expected number")),
    );
  }
  ...
  // Unknown top-level key → warning (not error)
  {
    ...
    logResult(
      "ck: unknown key produces warning",
      r.valid &&
        r.warnings.some((w) => w.includes("typo") && w.includes("unknown")),
    );
  }
  ...
  // Nested unknown key → warning
  {
    ...
    logResult(
      "ck: nested unknown key produces warning",
      r.valid && r.warnings.some((w) => w.includes("workflow.typo")),
    );
  }
  ...
  // Boolean field wrong type
  {
    ...
    logResult(
      "ck: wrong type for boolean rejected",
      !r.valid && r.errors.some((e) => e.includes("expected boolean")),
    );
  }
  ...
  // Array with wrong item type
  {
    ...
    logResult(
      "ck: array wrong item type rejected",
      !r.valid && r.errors.some((e) => e.includes("expected string")),
    );
  }
  ...
  // Real .ck.json validation
  {
    ...
    if (fs.existsSync(ckPath)) {
      ...
      logResult(
        "ck: real .ck.json passes schema",
        r.valid,
        r.errors.join("; "),
      );
    } else {
      logResult(
        "ck: real .ck.json exists",
        false,
        ".claude/.ck.json not found",
      );
    }
  }
}
