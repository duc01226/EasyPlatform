...
const designSystemContextTests = [
  {
    ...
    fn: async () => {
      ...
      try {
        ...
        assertTrue(
          output.includes('Design System') ||
          output.includes('WebV2') ||
          output === '',
          'May inject design system context'
        );
      } finally {
  ...
  },
  {
    ...
    fn: async () => {
      ...
      try {
        ...
        const output = result.stdout;
        assertTrue(
          !output.includes('Design System') || output === '',
          'Should not inject for non-frontend file'
        );
      } finally {
        cleanupTempDir(tmpDir);
      }
    ...
    fn: async () => {
      ...
      try {
        const input = createPreToolUseInput('Write', {
          file_path: 'src/PlatformExampleAppWeb/apps/playground-text-snippet/new.scss',
          content: '.btn { }'
        });
        const result = await runHook(DESIGN_SYSTEM_CONTEXT, input, { cwd: tmpDir });
        assertAllowed(result.code);
...
const backendCsharpContextTests = [
  ...
  },
  {
    ...
    fn: async () => {
      ...
      try {
        const input = createPreToolUseInput('Edit', {
          file_path: 'src/PlatformExampleApp/TextSnippet.Api/Controller.cs',
          old_string: 'a',
          new_string: 'b'
        });
        const result = await runHook(BACKEND_CSHARP_CONTEXT, input, { cwd: tmpDir });
    ...
    fn: async () => {
      ...
      try {
        ...
        assertTrue(
          ...
          output.includes('Frontend') ||
          output === '',
          'May inject platform-core context'
        );
      } finally {
        cleanupTempDir(tmpDir);
      }
...
const scssContextTests = [
  {
    ...
    fn: async () => {
      ...
      try {
        ...
        const output = result.stdout;
        assertTrue(
          output.includes('SCSS') ||
          output.includes('Styling') ||
          output === '',
          'May inject SCSS context'
        );
      } finally {
        cleanupTempDir(tmpDir);
      }
...
module.exports = {
  ...
  tests: [
    ...designSystemContextTests,
    ...backendCsharpContextTests,
    ...frontendTypescriptContextTests,
    ...scssContextTests
  ]
};
