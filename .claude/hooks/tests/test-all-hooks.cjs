...
function skipTest(name, reason) {
  console.log(
    `${COLORS.yellow}○${COLORS.reset} ${name}: ${COLORS.dim}${reason}${COLORS.reset}`,
  );
  results.skipped++;
}
  ...
  // Test 1: Startup trigger
  {
    ...
    if (result.stdout) {
      logOutputValidation(
        ...
        containsSystemReminder(result.stdout),
      );
    }
  }
...
async function testPostCompactRecovery() {
  ...
  // Test 3: Startup (should skip)
  {
    ...
    logResult(
      "Startup trigger exits 0 (no recovery needed)",
      result.code === 0,
    );
  }
...
async function testProjectConfigInit() {
  ...
  // Test 1: When populated config exists, should exit silently
  {
    ...
    logResult(
      "No AI directive when config populated",
      !result.stdout.includes("AI ACTION REQUIRED"),
    );
  }
  ...
  // Test 2: When config is missing, should create skeleton and suggest /project-config
  {
    ...
    try {
      ...
      logResult(
        "Silent creation (no verbose output)",
        !result.stdout.includes("Project Config Initialized"),
      );
      // Advisory text removed — enforcement is now in init-prompt-gate.cjs (exit 2)
      logResult(
        "No advisory text (gate handles enforcement)",
        !result.stdout.includes("MANDATORY MUST"),
      );
      ...
      if (configExists) {
        ...
        logResult(
          "Skeleton has contextGroups",
          Array.isArray(config.contextGroups),
        );
        ...
        logResult(
          "Skeleton has referenceDocs",
          Array.isArray(config.referenceDocs),
        );
      }
    } finally {
  ...
  // Test 3: Creates docs/ directory if missing (requires content dir for guard)
  {
    ...
    try {
      ...
      logResult(
        ...
        fs.existsSync(path.join(tmpDir, "docs")),
      );
      logResult(
        "Creates config inside docs/",
  ...
  // Test 5: Persistent — second run STILL suggests when skeleton is unpopulated
  {
    ...
    try {
      ...
      logResult(
        "Second run silent (gate handles enforcement)",
        !result.stdout.includes("MANDATORY MUST"),
      );
    } finally {
      cleanupTempDir(tmpDir);
  ...
  // Test 6: No suggestion when config is populated with real values
  {
    ...
    try {
      ...
      logResult(
        "No AI directive when populated",
        !result.stdout.includes("AI ACTION REQUIRED"),
      );
    } finally {
      cleanupTempDir(tmpDir);
...
async function testSubagentInit() {
  ...
  for (const subagentType of subagentTypes) {
    ...
    if (result.stdout) {
      ...
      logOutputValidation(
        `${subagentType} output valid`,
        parsed.valid || !result.stdout.includes("{"),
      );
    }
  }
...
async function testInitPromptGate() {
  ...
  // Test 2: Unpopulated config → exit 2 (blocked)
  {
    ...
    try {
      ...
      logResult(
        "Block message mentions /project-config",
        result.stderr.includes("/project-config"),
      );
      logResult(
        "Block message mentions skip init",
        result.stderr.includes("skip init"),
      );
    } finally {
      cleanupTempDir(tmpDir);
    }
  ...
  // Test 5: "skip init" → writes dismiss flag + exit 0
  {
    ...
    try {
      ...
      const flagExists = fs.existsSync(path.join(claudeDir, ".init-dismissed"));
  ...
  // Test 6: Active dismiss flag → exit 0 even with unpopulated config
  {
    ...
    try {
      ...
      // Write a fresh dismiss flag
      fs.writeFileSync(
        path.join(claudeDir, ".init-dismissed"),
  ...
  // Test 7: Expired dismiss flag (>1 day old) → exit 2
  {
    ...
    try {
      ...
      // Write a dismiss flag and backdate it to 2 days ago
      const flagPath = path.join(claudeDir, ".init-dismissed");
...
async function testLessonsInjector() {
  ...
  // Test 3: Lessons file with header only (no entries)
  {
    ...
    try {
      ...
      logResult(
        "Header-only lessons file exits 0, no output",
  ...
  // Test 4: Lessons file with entries outputs content
  {
    ...
    try {
      ...
      logResult(
        "Lessons with entries outputs content",
        result.code === 0 && result.stdout.includes("## Learned Lessons"),
      );
      logResult(
        "Output includes lesson entries",
...
async function testLessonLearnedReminder() {
  ...
  // Test 1: Returns reminder when no transcript
  {
    ...
    logResult(
      "Returns reminder with no transcript",
      result !== null && result.includes("[LESSON-LEARNED-REMINDER]"),
    );
  ...
  // Test 2: Dedup — returns null when marker in recent transcript
  {
    ...
    fs.writeFileSync(
      transcriptPath,
      "[LESSON-LEARNED-REMINDER] Task Planning\n".repeat(5),
    );
    try {
      const result = injectLessonReminder(transcriptPath);
      logResult(
        "Dedup: returns null when marker in transcript",
        result === null,
      );
    } finally {
      cleanupTempDir(tempDir);
  ...
  // Test 3: No dedup when transcript has no marker
  {
    ...
    try {
      const result = injectLessonReminder(transcriptPath);
      logResult(
        "Returns reminder when no marker",
        result !== null && result.includes("[LESSON-LEARNED-REMINDER]"),
      );
    } finally {
      cleanupTempDir(tempDir);
...
async function testWindowsCommandDetector() {
  ...
  for (const { cmd, name, shouldBlock } of windowsPatterns) {
    ...
    if (shouldBlock) {
      logOutputValidation(
        `${name} has block message`,
        result.stderr.includes("Windows CMD"),
      );
    }
  }
  ...
  logSubsection("Backslash-Bang Rewrite (\\! -> !)");
  {
    ...
    logResult(
      "updatedInput has fixed command",
      parsed.updatedInput?.command === 'node -e "console.log(!true)"',
    );
  }
  {
    ...
    logResult(
      'node -e "if(\\!x)" rewrites',
      parsed.updatedInput?.command === 'node -e "if(!x) {}"',
    );
  }
  {
    ...
    const result = await runHook("windows-command-detector.cjs", {
      ...
      tool_input: {
        command:
          'node --experimental-vm-modules -e "arr.filter(x => \\!x.done)"',
      },
    });
    let parsed = {};
    try {
    ...
    logResult(
      'node --flag -e "\\!" rewrites',
      parsed.updatedInput?.command ===
        'node --experimental-vm-modules -e "arr.filter(x => !x.done)"',
    );
  }
  {
    ...
    logResult(
      "single-quoted node -e not rewritten",
  ...
  }
  {
    ...
    logResult(
      "multiple \\! all replaced",
      parsed.updatedInput?.command === 'node -e "if(!a && !b) {}"',
    );
  }
  ...
  // (projectHasCode() checks for non-docs folders at CWD root — docs-only projects skip blocking)
  {
    ...
    try {
      // Test 5: Non-exempt .ts file without tasks - should BLOCK (exit 1)
      {
        ...
        logResult(
          "Non-exempt .ts file without tasks blocked",
          result.code === 1,
        );
      }
      ...
      // Test 6: Non-exempt .cs file without tasks - should BLOCK (exit 1)
      {
        ...
        logResult(
          "Non-exempt .cs file without tasks blocked",
          result.code === 1,
        );
      }
      ...
      // Test 7: MultiEdit tool - extracts primaryPath from edits[0].file_path → non-exempt → BLOCK
      {
        ...
        logResult(
          "MultiEdit non-exempt files without tasks blocked",
          result.code === 1,
        );
      }
      ...
      // Test 8: NotebookEdit tool - non-exempt without tasks → BLOCK
      {
        ...
        logResult(
          "NotebookEdit non-exempt without tasks blocked",
          result.code === 1,
        );
      }
    } finally {
...
async function testContextInjectors() {
  ...
  for (const filePath of frontendPaths) {
    ...
    if (result.stdout) {
      logOutputValidation(
        ...
        result.stdout.includes("design") || result.stdout.length < 10,
      );
    }
  }
  ...
  for (const filePath of backendPaths) {
    ...
    logResult(
      `Backend context: ${filePath.split("/").pop()}`,
      result.code === 0,
    );
  }
  ...
  for (const filePath of tsPaths) {
    ...
    logResult(
      `Frontend context: ${filePath.split("/").pop()}`,
      result.code === 0,
    );
  }
...
async function testNotification() {
  ...
  // Permission prompt via message only (missing notification_type) - should be skipped
  {
    ...
    logResult(
      "Message-based permission detection (skipped)",
      result.code === 0,
    );
  }
  ...
  // Regular message mentioning "permission" but not a permission prompt - should NOT be skipped
  {
    ...
    logResult(
      'Non-permission message with "permission" word (not skipped)',
      result.code === 0,
    );
  }
}
...
async function testDedupConstants() {
  ...
  logResult(
    "Exports CODE_PATTERNS (non-empty string)",
    typeof constants.CODE_PATTERNS === "string" &&
      constants.CODE_PATTERNS.length > 0,
  );
  logResult(
    "Exports LESSON_LEARNED (non-empty string)",
    typeof constants.LESSON_LEARNED === "string" &&
      constants.LESSON_LEARNED.length > 0,
  );
...
async function runAllTests() {
  ...
  if (VALIDATE_OUTPUT)
    console.log(`${COLORS.cyan}Output validation enabled${COLORS.reset}`);
  if (VERIFY_STATE)
    console.log(`${COLORS.cyan}State verification enabled${COLORS.reset}`);
  console.log();
  ...
  if (
    ...
    "prompt".includes(FILTER) ||
    "init".includes(FILTER)
  ) {
  ...
  if (VALIDATE_OUTPUT) {
    console.log(
      `${COLORS.cyan}Output Validations:${COLORS.reset} ${results.outputValidation.passed} passed, ${results.outputValidation.failed} failed`,
    );
  }
  if (VERIFY_STATE) {
    console.log(
      `${COLORS.cyan}State Verifications:${COLORS.reset} ${results.stateVerification.passed} passed, ${results.stateVerification.failed} failed`,
    );
  }
