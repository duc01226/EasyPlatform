...
async function testProjectConfigInit() {
    ...
    {
        const result = await runHook('session-init-docs.cjs', { source: 'startup' });
    ...
    // Test 6: No suggestion when config is populated with real values
    {
        ...
        try {
            ...
            const populatedConfig = {
                schemaVersion: 2,
                project: { name: 'MyProject', description: 'A real project', languages: ['TypeScript'], packageManagers: ['npm'] },
                backendServices: { patterns: [], serviceMap: { RealService: 'src/services/real' }, serviceRepositories: {}, serviceDomains: {} },
                frontendApps: { patterns: [], appMap: {}, legacyApps: [], modernApps: [], frontendRegex: '', sharedLibRegex: '' },
                designSystem: { docsPath: 'docs/project-reference/design-system', appMappings: [] },
                scss: { appMap: {}, patterns: [] },
                componentFinder: { selectorPrefixes: [], layerClassification: { platform: [], common: [], domain: [] } },
...
async function testInitPromptGate() {
    ...
    {
        const result = await runHook('init-prompt-gate.cjs', { prompt: 'implement feature X' });
    ...
    // Test 6: Active dismiss flag → exit 0 even with unpopulated config
    {
        ...
        try {
            ...
            fs.writeFileSync(path.join(claudeDir, '.init-dismissed'), new Date().toISOString());
            const result = await runHook(
                'init-prompt-gate.cjs',
                { prompt: 'implement feature X' },
                {
...
async function testDevRulesReminder() {
    ...
    // Test 2: Empty prompt
    {
        const result = await runHook('prompt-context-assembler.cjs', { prompt: '' });
...
async function testWindowsCommandDetector() {
    ...
    }
    {
        ...
        const result = await runHook('windows-command-detector.cjs', {
            tool_name: 'Bash',
            tool_input: { command: 'node --experimental-vm-modules -e "arr.filter(x => \\!x.done)"' }
        });
        let parsed = {};
        try {
            parsed = JSON.parse(result.stdout.trim());
...
async function testContextInjectors() {
    ...
    const backendPaths = [...fixtures.backendPaths, fixtures.platformCs];
