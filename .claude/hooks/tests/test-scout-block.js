...
const testCases = [
    ...
    },
    {
        ...
        input: {
            tool_name: 'Grep',
            tool_input: { pattern: 'test', path: 'node_modules' }
        },
        expected: 'BLOCKED'
    },
    {
    ...
    },
    {
        ...
        input: {
            tool_name: 'Read',
            tool_input: { file_path: 'node_modules/package.json' }
        },
        expected: 'BLOCKED'
    },
    ...
    // Subfolder blocking (THE BUG FIX)
    {
        ...
        input: {
            tool_name: 'Bash',
            tool_input: { command: 'ls packages/web/node_modules' }
        },
        expected: 'BLOCKED'
    },
    {
        ...
        input: {
            tool_name: 'Read',
            tool_input: { file_path: 'apps/api/node_modules/pkg/index.js' }
        },
        expected: 'BLOCKED'
    },
    ...
    // Python commands - should be ALLOWED (python is a dev tool like go, cargo, dotnet)
    {
        name: 'Bash: python code_graph build',
        input: {
            tool_name: 'Bash',
            tool_input: { command: 'python .claude/scripts/code_graph build --json' }
        },
        expected: 'ALLOWED'
    },
    {
        name: 'Bash: python3 script',
        input: {
            tool_name: 'Bash',
            tool_input: { command: 'python3 manage.py collectstatic' }
        },
        expected: 'ALLOWED'
    },
    {
        name: 'Bash: pip install',
        input: {
            tool_name: 'Bash',
            tool_input: { command: 'pip install tree-sitter' }
        },
        expected: 'ALLOWED'
    },

    // Grep pattern with blocked dir name - should be ALLOWED (pattern is search text, not a path)
    {
        name: 'Grep: pattern "build" (search text, not path)',
        input: { tool_name: 'Grep', tool_input: { pattern: 'build', path: 'src' } },
        expected: 'ALLOWED'
    },
    {
        name: 'Grep: pattern "dist" (search text, not path)',
        input: {
            tool_name: 'Grep',
            tool_input: { pattern: 'dist', path: '.claude' }
        },
        expected: 'ALLOWED'
    },
    {
        name: 'Grep: pattern "node_modules" (search text, not path)',
        input: {
            tool_name: 'Grep',
            tool_input: { pattern: 'node_modules', path: 'src' }
        },
        expected: 'ALLOWED'
    },

    // Grep with blocked PATH (not pattern) - should still be BLOCKED
    {
        name: 'Grep: path is node_modules (still blocked)',
        input: {
            tool_name: 'Grep',
            tool_input: { pattern: 'test', path: 'node_modules' }
        },
