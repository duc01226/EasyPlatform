...
function main() {
    try {
        ...
        if (toolName === 'TaskUpdate') {
            ...
            // Append to lastTodos (keep last 60, non-completed first)
            state.lastTodos = state.lastTodos || [];
            state.lastTodos.push({ content: subject, status: 'pending', activeForm, taskId });
            if (state.lastTodos.length > 60) {
                state.lastTodos = state.lastTodos.slice(-60);
            }

            // Track taskId → subject for workflow-task-guard enforcement
            if (taskId) {
                state.taskSubjects = state.taskSubjects || {};
                state.taskSubjects[taskId] = subject;
            }
