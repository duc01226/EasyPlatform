...
'use strict';
/**
 * SubagentStart Hook — Coding Patterns & Agent-Specific Docs page 4/5 (fires 5th of 13)
 *
 * Outputs: patterns page 4 of 5. Exits silently if content fits in pages 1-3.
 *
 * Execution order: identity → patterns-p1..p3 → patterns-p4 (this) → patterns-p5
...
async function main() {
    try {
        ...
        emitSubagentContext(buildPatternsContextPart(3, 5, agentType));
    } catch (error) {
        console.error(`SubagentStart patterns-p4 error: ${error.message}`);
