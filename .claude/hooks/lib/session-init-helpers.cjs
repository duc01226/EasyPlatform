...
const SKELETON = {
  _description:
    "Project-specific configuration consumed by .claude hooks at runtime. Update when adding services/apps.",
  schemaVersion: 2,
  project: {
    name: "",
    description: "",
...
const DEFAULT_REFERENCE_DOCS = [
  ...
  },
  {
    ...
    purpose:
      "SCSS/CSS styling guide: BEM methodology, mixins, variables, theming, responsive patterns.",
    sections: [
      "BEM Methodology",
      "SCSS Architecture",
      "Mixins & Variables",
  ...
  },
  {
    ...
    purpose:
      "Design system index: app-to-doc mapping, design tokens overview, component inventory.",
    sections: [
  ...
  },
  {
    ...
    purpose:
      "E2E test patterns: framework architecture, page objects, test configuration, and best practices.",
    sections: [
  ...
  },
  {
    ...
    purpose:
      "Domain entities, data models, DTOs, aggregate boundaries, cross-service entity sync, and ER diagrams.",
    sections: [
      "Entity Catalog",
...
function generatePlaceholderContent(doc) {
  ...
  for (const section of sections) {
    lines.push(
      ...
      `## ${section}`,
      "",
      `<!-- Document your ${section.toLowerCase()} here -->`,
    );
  }
...
function refreshScanStaleFlag(staleDays = 60) {
  const flagPath = path.join(PROJECT_DIR, ".claude", ".scan-stale");
  try {
