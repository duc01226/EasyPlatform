...
const backendGroup = (config.contextGroups || []).find(
  (g) => g.name === "Backend Services",
);
...
function buildInjection(context, filePath, service, patternsAlreadyInjected) {
  ...
  if (serviceName) {
    lines.push(
      ...
      "",
      `Working in **${serviceName}** service:`,
      "",
    );
    ...
    const repoType =
      mod?.meta?.repository ||
      (config.backendServices?.serviceRepositories || {})[serviceName];
    const domain =
      mod?.description ||
      (config.backendServices?.serviceDomains || {})[serviceName];
    if (repoType || domain) {
...
async function main() {
  try {
    ...
    console.log(
      buildInjection(context, filePath, service, patternsAlreadyInjected),
    );
    process.exit(0);
  } catch (error) {
    process.exit(0);
