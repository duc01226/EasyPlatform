...
const scenarios = [
    // === THE BUG CASES - These MUST ATTENTION be BLOCKED ===
    {
    ...
    // === Build commands - MUST ATTENTION be ALLOWED ===
    {
    ...
    // === Safe operations - MUST ATTENTION be ALLOWED ===
    {
...
const bugFixFailed = scenarios
    .filter(s => s.desc.includes('[BUG FIX]'))
    .some(s => {
        try {
            execSync(`node "${hookPath}"`, {
