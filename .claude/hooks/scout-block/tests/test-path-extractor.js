...
const toolInputTests = [
    ...
    // Grep pattern should NOT be extracted as a path
    {
        input: { pattern: 'build' },
        toolName: 'Grep',
        expected: [],
        desc: 'Grep pattern "build" skipped (search text, not path)'
    },
    {
        input: { pattern: 'node_modules' },
        toolName: 'Grep',
        expected: [],
        desc: 'Grep pattern "node_modules" skipped (search text, not path)'
    },
    // Glob pattern should STILL be extracted (it IS a path pattern)
    {
        input: { pattern: '**/node_modules/**' },
        toolName: 'Glob',
        expected: ['**/node_modules/**'],
        desc: 'Glob pattern still extracted (is a file path pattern)'
    },
    {
...
for (const test of toolInputTests) {
    const result = extractFromToolInput(test.input, test.toolName);
    let success;

    if (test.expected) {
        // For empty expected arrays, check result is also empty
        success = test.expected.length === 0 ? result.length === 0 : test.expected.every(e => result.includes(e));
