...
const { loadConfig, writeEnv, writeSessionState, resolvePlanPath, getReportsPath, resolveNamingPattern } = require('./lib/ck-config-utils.cjs');
const { writeResetMarker } = require('./lib/context-tracker.cjs');
const { cleanupAll } = require('./lib/temp-file-cleanup.cjs');
const { loadState: loadWorkflowState, clearState: clearWorkflowState } = require('./lib/workflow-state.cjs');
...
async function main() {
    try {
        ...
        // Layer 4: Preserve workflow state on resume/compact
        // The workflow-state.cjs module handles persistence - we just ensure session ID is propagated
        const workflowState = sessionId ? loadWorkflowState(sessionId) : null;
        const hasActiveWorkflow = workflowState?.workflowType && workflowState?.workflowSteps?.length > 0;
        ...
        if (source === 'startup') {
            ...
            if (!fs.existsSync(copilotPath)) {
                ...
                console.log('');
                console.log('⚠️  **Copilot instructions not found:** `.github/copilot-instructions.md`');
                console.log('');
                console.log('To enable better Copilot support (sync Claude Code protocols):');
                console.log('');
        ...
        if (source === 'startup' && !staticEnv.pythonVersion) {
            console.log('\n## Python Not Found');
            console.log('');
            console.log('\u26a0\ufe0f  **Python is not installed or not in PATH.** Some skills require Python:');
            console.log('  \u2022 `generate_catalogs.py` \u2014 Skills/commands catalog generation');
            console.log('  \u2022 `ck-help.py` \u2014 /ck-help skill');
            console.log('  \u2022 `ai-multimodal` \u2014 Image/video/doc processing (Gemini)');
            ...
            if (process.platform === 'win32') {
                console.log('  `winget install Python.Python.3.12`  or  https://python.org/downloads');
            } else if (process.platform === 'darwin') {
                console.log('  `brew install python3`  or  https://python.org/downloads');
            } else {
                console.log('  `sudo apt install python3`  or  https://python.org/downloads');
            }
            console.log('');
            console.log('**Override:** Set `PYTHON_PATH` env var to your Python binary path.');
            console.log('');
        }
