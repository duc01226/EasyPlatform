 ...
 *   searchBeforeCode.requireSearchFor - File patterns requiring search (from project-config.json)
...
const { loadProjectConfig, getAllFileExtensions, isKnowledgePath } = require('./lib/project-config-loader.cjs');
...
const SEARCH_WINDOW_LINES = DEDUP_LINES.SEARCH_WINDOW;
const TRIVIAL_LINE_THRESHOLD = 20; // Files with fewer lines are considered trivial
const PROJECT_DIR = process.env.CLAUDE_PROJECT_DIR || process.cwd();

// File extensions that require pattern search (config-driven from contextGroups + styling)
const REQUIRE_SEARCH_EXTENSIONS = getAllFileExtensions();
// Explicit exclusion — never require search for these (safety net against future config drift)
...
const PATTERN_KEYWORDS = projectConfig.framework?.searchPatternKeywords || ['Command.*Handler', 'Repository'];
...
function hasSkipSearchOverride(transcriptContent) {
    if (!transcriptContent) return false;
    const recentLines = transcriptContent.split('\n').slice(-DEDUP_LINES.SEARCH_SKIP_OVERRIDE).join('\n');
...
function main() {
    try {
        ...
        // Get file content
        const newContent = payload.tool_input?.new_string || payload.tool_input?.content || payload.tool_input?.edits?.[0]?.new_string || '';
        ...
        const exampleExt = REQUIRE_SEARCH_EXTENSIONS[0] ? `*${REQUIRE_SEARCH_EXTENSIONS[0]}` : '*.*';
        console.error(`  grep -r "${exampleKeyword}" src/ --include="${exampleExt}" -A 3\n`);
