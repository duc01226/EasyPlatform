...
const { WORKFLOW_CATALOG: WORKFLOW_CATALOG_MARKER, DEDUP_LINES } = require('./lib/dedup-constants.cjs');
...
function buildCatalogInjection(config, quickMode) {
    ...
    lines.push('3. **ASK (MANDATORY):** ALWAYS use `AskUserQuestion` to present the user with choices:');
    lines.push('   - **"Activate [Workflow Name] (Recommended)"** — include workflow sequence as description');
    lines.push('   - **"Execute directly without workflow"** — skip workflow, proceed immediately');
    lines.push('4. **ACTIVATE (if confirmed):** Call `/workflow-start <workflowId>`, announce to user');
    lines.push('5. **TASKCREATE (MANDATORY):** Create `[Workflow]` items for each step BEFORE any other action');
    lines.push('');
    lines.push('Never auto-activate without asking. Never skip detection. User always confirms.');
    lines.push('');
    lines.push('### Plan Implementation (ALWAYS confirm)');
    lines.push('');
    lines.push('When the user asks to implement, execute, or start an existing plan (in any language), you MUST:');
    ...
    lines.push('3. Example: "Plan is already validated. Skip to `/cook`? Or follow full workflow sequence?"');
    lines.push('This applies regardless of task complexity — plan implementation is NEVER auto-activated without confirmation.');
    lines.push('');
    ...
    lines.push('');
    lines.push('**Rules:**');
    lines.push('1. Create ONE `TaskCreate` per workflow step — do NOT combine or skip steps');
    lines.push('2. The number of TaskCreate calls MUST EQUAL the number of steps in the workflow sequence');
    lines.push('3. After creating all tasks, call `TaskList` to VERIFY the count matches');
    lines.push('4. Mark each task `in_progress` (via `TaskUpdate`) before starting, `completed` after finishing');
    ...
    lines.push('7. On context loss, check `TaskList` for `[Workflow]` items to recover');
    lines.push('');

    lines.push('> **CRITICAL:** Task count MUST match step count. A 16-step workflow needs EXACTLY 16 TaskCreate calls. Fewer = bug.');
    lines.push('');
...
async function main() {
    try {
        ...
        // Skip if catalog was recently injected (prevents double-injection from SessionStart + UserPromptSubmit)
        ...
        // Skip catalog injection for explicit commands (not applicable for SessionStart)
        if (!isSessionStart && /^\/\w+/.test(effectivePrompt.trim())) {
            process.exit(0);
        }
