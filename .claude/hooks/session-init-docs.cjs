  ...
  try {
    ...
    for (const doc of referenceDocs) {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        created.push(
          `- \`docs/project-reference/${doc.filename}\` — ${doc.purpose || "Reference document"}`,
        );
      }
    }
    ...
    const placeholderDocs = referenceDocs
      ...
      .filter((doc) =>
        isPlaceholderFile(path.join(REF_DOCS_DIR, doc.filename)),
      );

    if (placeholderDocs.length > 0) {
      const skillList = placeholderDocs
        .map((d) => `/${SCAN_SKILL_MAP[d.filename]}`)
        .join(", ");
      console.log(
        `${placeholderDocs.length} reference doc(s) are placeholders. Run: ${skillList}`,
      );
    }
    ...
    const STALE_FLAG = path.join(PROJECT_DIR, ".claude", ".scan-stale");
    ...
    try {
      ...
      if (staleDocs.length > 0) {
        ...
        fs.writeFileSync(
          STALE_FLAG,
          JSON.stringify(
            { staleDays, docs: staleDocs, checkedAt: new Date().toISOString() },
            null,
            2,
          ) + "\n",
          "utf-8",
        ...
        // Advisory warning
        const docList = staleDocs
          .map(
            (d) =>
              `  - ${d.filename} (${d.ageDays}d old, last: ${d.lastScanned}) -> /${d.scanSkill}`,
          )
          .join("\n");
        console.log(
          [
