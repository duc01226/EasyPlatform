...
- ALWAYS state gate status explicitly: `PASS` / `FAIL` / `CONDITIONAL`
- Every critical item requires `file:line` evidence — NEVER assume — why: an unverified criterion silently defeats the gate
- NEVER pass a gate without verified evidence
- NEVER skip checklist items — verify all before declaring progression
...
> **MANDATORY MUST ATTENTION** — Read project-specific reference docs directly: `project-structure-reference.md`.
>
> Not found? Search: service directories, configuration files, project patterns.
...
NEVER mark a gate `PASS` while any of its items remain unverified — a single skipped item fails the whole gate.
...
**IMPORTANT MUST ATTENTION Goal:** Run quality gates on artifacts and code — verify each criterion against `file:line` evidence, track metrics, and emit an auditable `PASS`/`FAIL`/`CONDITIONAL` decision so only verified work passes to the next stage.
...
- **Double Round-Trip Review:** Validate findings, fix, fresh full re-review; clean pass ends loop.
- **Fresh Context Review:** Restart review with zero-memory sub-agents after each fix cycle.
- **Review Protocol Injection:** NEVER paraphrase — embed all 11 protocol bodies verbatim into every fresh sub-agent prompt.
- **Refinement DoR Checklist:** All 7 DoR criteria pass before grooming.
- **Estimation Framework:** Bottom-up hours first; SP derived; output range with risk margin.

**IMPORTANT MUST ATTENTION** NEVER pass a gate without `file:line` evidence for EVERY critical criterion — verify each, then declare — why: a gate passed on an unverified or hallucinated check is worse than no gate.
**IMPORTANT MUST ATTENTION** NEVER skip checklist items — verify ALL Pre-Dev / Pre-QA / Pre-Release criteria before progression; one skipped item fails the whole gate — why: a single unchecked criterion silently defeats the gate's purpose.
**IMPORTANT MUST ATTENTION** ALWAYS declare gate status explicitly: `PASS` / `FAIL` / `CONDITIONAL` — never leave the verdict implicit — why: downstream stages branch on the explicit decision.
**IMPORTANT MUST ATTENTION** select the correct gate (Pre-Dev / Pre-QA / Pre-Release) from artifact type or workflow stage BEFORE verifying — why: the wrong checklist passes wrong work.
...
**IMPORTANT MUST ATTENTION** classify findings by the Severity Rubric (Critical/High/Medium/Low) by consequence — Critical/High block `PASS` until fixed or owner-accepted — why: one scale keeps "High" meaning the same everywhere.
...
**IMPORTANT MUST ATTENTION** for fix-then-re-review cycles, validate findings first, fix only validated ones, then run a FRESH full re-review — a clean pass ENDS the loop — why: every fix invalidates the prior verdict.
...
**IMPORTANT MUST ATTENTION** top-3 (primacy-recency echo): (1) NEVER `PASS` without `file:line` evidence for every critical criterion; (2) NEVER skip checklist items — verify ALL before progression; (3) ALWAYS declare gate status explicitly `PASS` / `FAIL` / `CONDITIONAL`.
