...
> **`/plan-review`** — Auto-reviews plan for validity, correctness, and best practices. Recursive: fixes actionable findings directly in plan files and re-reviews until a complete pass has zero findings, unless the same blocker repeats 3 times with no progress. Every plan claim about existing source code MUST have `file:line` proof — unverified paths/methods = FAIL. Plans must be detailed and small enough (≤5 files, ≤3h per phase). MUST ATTENTION run after every plan creation.
