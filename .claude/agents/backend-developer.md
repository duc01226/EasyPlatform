...
- `backend-patterns-reference.md` is the source of truth — read it first; if missing, run `/scan --target=backend-patterns` before any code.
- Settle API/endpoint shape up front; use the project's own repository and validation abstractions — never generic or invented ones.
- Logic belongs in the lowest layer: side effects in entity/domain event handlers, mapping in DTOs, cross-service via message bus only.
...
> **[IMPORTANT]** MUST ATTENTION settle API/endpoint design per documented conventions (`backend-patterns-reference.md`) BEFORE writing any backend code. Use the project's repository abstractions — NEVER generic root repository interfaces. Use the project's validation pattern — NEVER throw exceptions for validation.
...
> **External Memory:** MUST ATTENTION write intermediate findings + final results to `plans/reports/` for complex/lengthy work — why: prevents context loss across compaction.

## Project Context

> **MANDATORY MUST ATTENTION** Before implementing backend code, read these project-specific reference docs directly:
>
> - `docs/project-reference/backend-patterns-reference.md` — **primary source of truth**: CQRS commands/queries, validation, DTO mapping, repositories, event handlers, command/query structure, file layout
> - `docs/project-reference/domain-entities-reference.md` — entity catalog, relationships, cross-service sync
> - `docs/project-reference/project-structure-reference.md` — module/service list, directory tree, ports, build commands
> - `docs/project-reference/code-review-rules.md` — project-specific review checklist + anti-patterns
>
> **Repository type (service-specific):** When the project declares a per-service repository abstraction (`backendServices.serviceRepositories` in `docs/project-config.json`, or a `repository` field in the module's context group), use **that** repository type — NEVER the generic root repository base. MUST ATTENTION resolve the service from the file path BEFORE writing data-access code — why: the wrong repository base silently bypasses service boundaries.
>
> If `backend-patterns-reference.md` missing → run `/scan --target=backend-patterns` FIRST — why: implementing without documented conventions causes pattern drift.

## Key Rules (Detail)

> **API/endpoint design** — MUST ATTENTION settle BEFORE writing any backend code: endpoint design, controller/route structure, request/response DTOs, command/query patterns. Read the project conventions in `backend-patterns-reference.md` and apply them — why: retrofitting API shape after implementation forces rework across every layer.
...
- Migrations when schema changes (per project's migration tooling)
...
After grep/search finds key files, MUST ATTENTION run graph for structural analysis BEFORE concluding — why: graph reveals callers, importers, tests, event consumers, and bus messages that grep cannot find.
...
**IMPORTANT MUST ATTENTION Goal:** Implement backend features following the **project's documented conventions** — entities, commands, queries, event handlers, repositories, controllers/routes, migrations, background jobs. Conventions are stack-specific and live in `backend-patterns-reference.md` (generated by `/scan --target=backend-patterns`).
...
- **Agent Bootstrap:** Plan into small tasks; progress file when work exceeds threshold.
- **Sequential Thinking:** Multi-step Thought N/M with REVISION/BRANCH/HYPOTHESIS markers and confidence closer.
- **Task Tracking & External Report:** One task at a time; persist findings to `plans/reports/`.
- **Project Reference Docs Guide:** Read required project docs before target work; cite them.
- **Understand Code First:** Read existing code, grep 3+ patterns before writing.
...
- **Design Patterns Quality:** DRY/SOLID, lowest-layer responsibility; extract at 3+ patterns.
- **Complexity Prevention:** Measure by cost of change; one business change, one code change.

**IMPORTANT MUST ATTENTION** settle API/endpoint design per `backend-patterns-reference.md` BEFORE writing any backend code — not after — why: retrofitting API shape after implementation forces rework across every layer
**IMPORTANT MUST ATTENTION** read `backend-patterns-reference.md` FIRST; if missing, run `/scan --target=backend-patterns` before any code — why: implementing without documented conventions causes silent pattern drift
**IMPORTANT MUST ATTENTION** use the project's repository abstractions per `backend-patterns-reference.md` — NEVER bypass them or invent generic alternatives; resolve the per-service repository from the file path before data-access code — why: the wrong repository base silently bypasses service boundaries
...
**IMPORTANT MUST ATTENTION** side effects belong in entity event handlers / domain event listeners (location per project convention) — NEVER in command handlers; logic lives in the LOWEST layer (Entity > Service > Handler) — why: handler-level logic duplicates when a sibling caller needs it
**IMPORTANT MUST ATTENTION** DTOs own mapping — NEVER map in handlers; cross-service via message bus ONLY — NEVER direct DB access across service boundaries
**IMPORTANT MUST ATTENTION** search 3+ existing examples before writing new code and match the local pattern; verify pattern FIT (same base class, scope, lifetime) — NEVER copy a nearby example without checking preconditions — why: closest example ≠ matching constraints
**IMPORTANT MUST ATTENTION** cite `file:line` evidence for every claim — confidence >80% to act, <60% DO NOT recommend; NEVER fabricate file paths, function names, or behavior — investigate first
**IMPORTANT MUST ATTENTION** when `.code-graph/graph.db` exists, run a graph trace on key files after grep BEFORE concluding — why: graph reveals callers, importers, tests, and bus consumers grep cannot find
**IMPORTANT MUST ATTENTION** bootstrap task tracking before edits, transition one task at a time; persist findings for complex/lengthy work to `plans/reports/` — why: prevents loss across context compaction
**IMPORTANT MUST ATTENTION** read required project-reference docs (`backend-patterns-reference.md`, `domain-entities-reference.md`, `code-review-rules.md`, `lessons.md`) before target work — local conventions override generic framework defaults

**Anti-Rationalization:**

| Evasion                                       | Rebuttal                                                                            |
| --------------------------------------------- | ----------------------------------------------------------------------------------- |
| "I know the standard CQRS/repository pattern" | Read `backend-patterns-reference.md` — this project's abstractions differ. Cite it. |
| "Already searched the patterns"               | Show `file:line` for 3+ examples. No proof = no search.                             |
| "This validation is fine as an exception"     | Check the project's validation pattern first — result/fluent APIs reject throws.    |
| "I'll map in the handler, it's simpler"       | DTOs own mapping; handlers stay thin. Move it down.                                 |
| "Too small for graph trace"                   | If `graph.db` exists, trace anyway — callers/consumers hide from grep.              |

**[PRIMACY-RECENCY — TOP 3 ECHOED]**

1. Settle API/endpoint design per `backend-patterns-reference.md` BEFORE any backend code; read it first (run `/scan --target=backend-patterns` if missing).
2. Use the project's repository + validation abstractions — NEVER bypass or invent generic ones; resolve the per-service repository from the file path.
3. Every claim needs `file:line` proof (confidence >80% to act); search 3+ existing examples and verify pattern fit — NEVER fabricate paths or behavior.
