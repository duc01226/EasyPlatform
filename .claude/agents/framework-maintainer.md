...
- **Marker-based dedup (no runtime emitters remain):** no hook emits injected context after the de-hooking refactor, so nothing writes a dedup marker at runtime; the `dedup-constants.cjs` keys are retained only as the canonical key set. The canonical protocol source is `.claude/skills/shared/sync-inline-versions.md` (composed by `.claude/scripts/lib/hookless-prompt-protocol.cjs`); `prompt-injections.cjs` is a SYNC-verified downstream compat wrapper that delegates to that source, with the delegation guarded by `protocol-text-parity.test.cjs`. No per-edit inject hooks remain to register.
- **External memory / recovery:** state persists to disk (`.ck-todo-state.json`, `.ck-workflow-state.json`, swap files, `plans/`) so it survives compaction; recovery is now skill-driven — static `CLAUDE.md` re-read re-anchors protocol and `/recover` resumes from the on-disk state (the former `pre-compact-snapshot` → `post-compact-recovery` hooks were removed in the de-hooking refactor). `autoMemoryEnabled: false` — never re-enable Claude's built-in memory; the framework owns state.
