...
1. **Verify ignored** — Project-local worktree directories (`.worktrees/`, `worktrees/`) must be in `.gitignore` before creating worktrees. Use `git check-ignore -q <dir>` to verify.
2. **Baseline tests** — Run tests in new worktree before making changes to establish clean baseline. If tests fail, report and ask before proceeding.
3. **Project setup** — Auto-detect and run setup commands in new worktrees (`npm install`, `dotnet restore`, etc.)
4. **Report location** — After setup, report worktree path, baseline test results, and readiness status.
