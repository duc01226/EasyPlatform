...
- **Performance Best Practices:** No O(n²) nested loops (use dictionary/lookup); Project only needed properties in query (not GetAll then Select); Always paginate large datasets (never GetAll without paging); Use parallel queries for independent operations.
