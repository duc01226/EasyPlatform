---
agent: agent
description: Easy.Platform backend development - CQRS commands/queries, entities, validation, side effects, migrations, background jobs, and message bus patterns.
---
...
**`docs/claude/backend-csharp-complete-guide.md`** - Complete patterns for CQRS, validation, repositories, entities, jobs, migrations
...
```
{Service}.Application/
├── UseCaseCommands/{Feature}/Save{Entity}Command.cs     # Command+Handler+Result (ONE FILE)
...
- `"*/5 * * * *"` - Every 5 minutes
- `"0 3 * * *"` - Daily 3 AM
- `"5 0 * * *", executeOnStartUp: true` - Daily + startup
...
protected override async Task<PlatformValidationResult<TCommand>> ValidateRequestAsync(
    ...
    => await validation
        .AndAsync(r => repo.GetByIdsAsync(r.Ids, ct)
            .ThenValidateFoundAllAsync(r.Ids, ids => $"Not found: {ids}"))
        .AndNotAsync(r => repo.AnyAsync(e => e.IsExternal && r.Ids.Contains(e.Id), ct),
            "External items not allowed");
```
...
- [ ] Entity Event Handler signature: `public override async Task<bool> HandleWhen()`
...
- CQRS advanced patterns (validation, async validation, parallel operations)
- Entity patterns (computed properties, async expressions, audited entities)
- Repository extensions and query patterns
- Migration patterns (EF Core, MongoDB, cross-DB sync)
- Background job coordination (batch scrolling, master-child)
