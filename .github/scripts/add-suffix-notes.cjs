#!/usr/bin/env node
/**
 * Add suffix notes to Copilot prompts and agents
 * Usage: node add-suffix-notes.cjs [--dry-run] [--type=prompts|agents|all]
 *
...
// Check for both old and new formats to ensure idempotency
 ...
 */
function findFiles(baseDir, extension) {
    const results = [];
...
function main() {
    ...
    console.log('Add Suffix Notes to Copilot Prompts and Agents');
    console.log('='.repeat(60));
    ...
    if (type === 'all' || type === 'prompts') {
        ...
        const promptFiles = findFiles(promptsDir, '.prompt.md');
        console.log(`\n📁 Processing ${promptFiles.length} prompt files...`);
