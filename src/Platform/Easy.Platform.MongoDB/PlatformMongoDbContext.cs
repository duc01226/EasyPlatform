    ...
    where TDbContext : PlatformMongoDbContext<TDbContext>, IPlatformDbContext<TDbContext>
{
    ...
    public IMongoCollection<PlatformMongoMigrationHistory> MigrationHistoryCollection => Database.GetCollection<PlatformMongoMigrationHistory>(MigrationHistoryCollectionName);
        ...
        where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        ...
        var toBeCreatedEntities = entities.SelectList(entity =>
            entity
                ...
                .WithIf(entity is IRowVersionEntity { ConcurrencyUpdateToken: null }, entity => entity.As<IRowVersionEntity>().ConcurrencyUpdateToken = Ulid.NewUlid().ToString())
        );
                    ...
                    if (result.MatchedCount != hasDataChangedToBeUpdatedItems.Count)
                    {
                        ...
                        if (hasDataChangedToBeUpdatedItems.First().toBeUpdatedEntity is IRowVersionEntity)
                        {
                            ...
                            hasDataChangedToBeUpdatedItems.ForEach(p =>
                            {
                                ...
                                {
                                    throw new PlatformDomainRowVersionConflictException($"Update {typeof(TEntity).Name} with Id:{p.toBeUpdatedEntity.Id} has conflicted version.");
                                }
                            });
                        }
        ...
        where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        ...
        return await entities
            ...
            .ThenActionAsync(entities => SendBulkEntitiesEvent<TEntity, TPrimaryKey>(entities, PlatformCqrsEntityEventCrudAction.Deleted, eventCustomConfig, cancellationToken))
            .Then(entities =>
            {
                entities.ForEach(p => MappedUnitOfWork?.RemoveCachedExistingOriginalEntity(p.Id.ToString()));
                return entities;
        ...
        where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        if (entities.Any())
        {
            ...
            var existingEntitiesQuery = GetQuery<TEntity>()
                .Pipe(query =>
                    customCheckExistingPredicateBuilder != null || entities.FirstOrDefault()?.As<IUniqueCompositeIdSupport<TEntity>>()?.FindByUniqueCompositeIdExpr() != null
                        ? query.Where(
                            entities
                                .Select(entity =>
    ...
    public Task<TEntity> UpdateAsync<TEntity, TPrimaryKey>(
        ...
        where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        return InternalUpdateOrSetAsync<TEntity, TPrimaryKey>(entity, existingEntity, dismissSendEvent, checkDiff, eventCustomConfig, onlySetData: false, cancellationToken);
    }
    ...
    public virtual async Task EnsureIndexesAsync(bool recreate = false)
    {
        ...
        {
            await MigrationHistoryCollection.InsertOneAsync(new PlatformMongoMigrationHistory(EnsureIndexesMigrationName));
        }
            ...
            await InboxBusMessageCollection.Indexes.CreateManyAsync(
                [
                    new CreateIndexModel<PlatformInboxBusMessage>(
                        Builders<PlatformInboxBusMessage>
                            .IndexKeys.Ascending(p => p.ConsumeStatus)
                    ...
                    new CreateIndexModel<PlatformInboxBusMessage>(Builders<PlatformInboxBusMessage>.IndexKeys.Ascending(p => p.ConsumeStatus).Ascending(p => p.CreatedDate)),
                ]
            );
        }
    }
    ...
    public virtual async Task EnsureOutboxBusMessageCollectionIndexesAsync(bool recreate = false)
    {
        ...
        if (recreate || !await IsEnsureIndexesMigrationExecuted())
        {
            await OutboxBusMessageCollection.Indexes.CreateManyAsync(
                [
                    new CreateIndexModel<PlatformOutboxBusMessage>(Builders<PlatformOutboxBusMessage>.IndexKeys.Ascending(p => p.SendStatus).Ascending(p => p.CreatedDate)),
                    new CreateIndexModel<PlatformOutboxBusMessage>(
                        Builders<PlatformOutboxBusMessage>.IndexKeys.Ascending(p => p.SendStatus).Ascending(p => p.NextRetryProcessAfter).Ascending(p => p.CreatedDate)
                    ),
                    new CreateIndexModel<PlatformOutboxBusMessage>(
                        Builders<PlatformOutboxBusMessage>.IndexKeys.Ascending(p => p.SendStatus).Ascending(p => p.LastProcessingPingDate).Ascending(p => p.CreatedDate)
                    ),
                ]
            );
        }
        ...
        where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        var toBeCreatedEntity = entity
            .PipeIf(entity.IsAuditedUserEntity(), p => p.As<IUserAuditedEntity>().SetCreatedBy(RequestContextAccessor.Current.UserId(entity.GetAuditedUserIdType())).As<TEntity>())
            .WithIf(entity is IRowVersionEntity { ConcurrencyUpdateToken: null }, entity => entity.As<IRowVersionEntity>().ConcurrencyUpdateToken = Ulid.NewUlid().ToString());
        ...
        else
        {
            await PlatformCqrsEntityEvent.ExecuteWithSendingCreateEntityEvent<TEntity, TPrimaryKey, TEntity>(
                ...
                MappedUnitOfWork,
                toBeCreatedEntity,
                entity => GetTable<TEntity>().ReplaceOneAsync(p => p.Id.Equals(entity.Id), entity, new ReplaceOptions { IsUpsert = true }, cancellationToken).Then(() => entity),
                dismissSendEvent,
                eventCustomConfig,
                () => RequestContextAccessor.Current.GetAllKeyValues(),
                PlatformCqrsEntityEvent.GetEntityEventStackTrace<TEntity>(RootServiceProvider, dismissSendEvent),
