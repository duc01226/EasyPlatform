...
public class PlatformDefaultApplicationRequestContext : IPlatformApplicationRequestContext
{
    ...
    public T GetValue<T>(string contextKey)
    {
        ...
        if (PlatformRequestContextHelper.TryGetValue(FullCachedRequestContextData, contextKey, out T? foundValue)) return foundValue;

        if (PlatformRequestContextHelper.TryGetValue(LazyLoadCachedRequestContextData, contextKey, out foundValue)) return foundValue;
