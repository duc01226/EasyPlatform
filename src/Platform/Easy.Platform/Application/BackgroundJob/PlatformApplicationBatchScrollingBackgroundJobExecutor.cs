...
/// public class AutoHandlePendingRequestsForApprovalProcessBackgroundJobV2
///     : PlatformApplicationBatchScrollingBackgroundJobExecutor<LeaveRequest, string>
/// {
///     // Repository access pattern
///     protected override Task<List<TSelector>> GetAllEntitiesAsync<TSelector>(
///         IServiceProvider serviceProvider,
///         Func<IQueryable<LeaveRequest>, IQueryable<TSelector>> queryBuilder,
///         CancellationToken cancellationToken = default,
///         params Expression<Func<LeaveRequest, object?>>[] loadRelatedEntities)
///     {
///         return serviceProvider.GetRequiredService<IGrowthRootRepository<LeaveRequest>>()
///             .GetAllAsync(queryBuilder, cancellationToken, loadRelatedEntities);
///     }
///
///     // Query building for entities with batch filtering
///     protected override IQueryable<LeaveRequest> EntitiesQueryBuilder(
///         IQueryable<LeaveRequest> query, object? param, string? batchKey = null)
///     {
///         return query.Where(
///             LeaveRequest.GetPendingRequestsForApprovalProcessBackgroundJobExpr()
///                 .AndAlso(lr => lr.Company!.Subscriptions!.AsQueryable()
///                     .Any(Subscription.HasActiveTimeManagementLicenseExpr(...)))
///                 .AndAlsoIf(batchKey != null, () => lr => lr.CompanyId == batchKey));
///     }
///
///     // Batch key discovery (company IDs)
///     protected override IQueryable<string> EntitiesBatchKeyQueryBuilder(
///         IQueryable<LeaveRequest> query, object? param, string? batchKey = null)
///     {
///         return EntitiesQueryBuilder(query, param, batchKey)
///             .Select(lr => lr.CompanyId)
///             .Distinct();
///     }
///
///     // Business logic processing
///     protected override async Task ProcessEntitiesAsync(
///         List<LeaveRequest> entities, string batchKey, object? param, IServiceProvider serviceProvider)
///     {
///         var helper = serviceProvider.GetRequiredService<AutoProcessPendingRequestsHelper>();
///         await entities.ParallelAsync(async request => {
    ...
    where TParam : class
{
    ...
    /// {
    ///     return query.Where(
    ///         LeaveRequest.GetPendingRequestsExpr()
    ///             .AndAlso(lr => lr.Company!.Subscriptions!.AsQueryable()
    ///                 .Any(Subscription.HasActiveLicenseExpr()))
    ///             .AndAlsoIf(batchKey != null, () => lr => lr.CompanyId == batchKey));
    /// }
    /// ```
    ///
    ...
    ///
    /// **Key Considerations**:
    /// - Use the appropriate service-specific repository (IGrowthRootRepository, etc.)
    ...
    ///     IServiceProvider serviceProvider,
    ///     Func<IQueryable<LeaveRequest>, IQueryable<TQueryItemResult>> queryBuilder,
    ///     CancellationToken cancellationToken = default)
    /// {
    ///     return serviceProvider.GetRequiredService<IGrowthRootRepository<LeaveRequest>>()
    ...
    /// phase within a single batch (e.g., leave requests within one company).
    ///
    ...
    {
        Logger.LogError(exception, "Batch {BatchKey} failed with error: {ErrorMessage}", batchKey, exception.Message);
    }
