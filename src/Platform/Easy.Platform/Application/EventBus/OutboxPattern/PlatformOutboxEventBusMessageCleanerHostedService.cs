...
{
    public abstract class PlatformOutboxEventBusMessageCleanerHostedService : PlatformIntervalProcessHostedService
    {
        /// <summary>
            ...
            using (var scope = serviceProvider.CreateScope())
            {
                ...
                using (var uow = uowManager!.Begin())
                {
                    ...
                    var expiredMessages = outboxEventBusMessageRepo!.GetAllQuery()
                        .Where(p => p.LastSendDate <= Clock.UtcNow.AddDays(-MessageExpiredInDays()) &&
                                    p.SendStatus == PlatformOutboxEventBusMessage.SendStatuses.Processed)
                    ...
                    {
                        await outboxEventBusMessageRepo.DeleteManyAsync(expiredMessages, dismissSendEvent: true, cancellationToken);

                        await uow.CompleteAsync(cancellationToken);

                        Log.Information(Logger, message: $"CleanOutboxEventBusMessage success. Number of deleted messages: {expiredMessages.Count}");
                    }
                }
            }
        }
    ...
    public class PlatformDefaultOutboxEventBusMessageCleanerHostedService : PlatformOutboxEventBusMessageCleanerHostedService
    {
