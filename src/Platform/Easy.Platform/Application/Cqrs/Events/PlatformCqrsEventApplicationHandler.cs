    ...
    where TEvent : PlatformCqrsEvent, new()
{
    ...
    protected override Func<IServiceProvider, TEvent, PlatformCqrsEventHandler<TEvent>, Task> ExecuteHandleInBackgroundNewScopeBeforeExecuteFn =>
        async (newScopeServiceProvider, @event, handlerNewInstance) =>
        {
            if (handlerNewInstance is PlatformCqrsEventApplicationHandler<TEvent> applicationHandlerNewInstance &&
                applicationHandlerNewInstance.RequestContext.Count < @event.RequestContext.Count)
    ...
    private void ProcessEventHandlerPipeLineInRequestContext(TEvent @event)
    {
        var requestContextEventHandlerPipeLine =
            @event.RequestContext.GetRequestContextValue<List<string>>(IPlatformCqrsEventApplicationHandler.RequestContextEventHandlerPipeLineKey) ?? [];
        ...
        if (IsHandlingInNewScope) RequestContext.Upsert(IPlatformCqrsEventApplicationHandler.RequestContextEventHandlerPipeLineKey, requestContextEventHandlerPipeLine);
    }
    ...
    {
        var routingKey = PlatformBusMessageRoutingKey.BuildDefaultRoutingKey(@event.GetType(), ApplicationSettingContext.ApplicationName);

        return $"{routingKey}-{GetType().GetNameOrGenericTypeName()}";
    }
