...
public static class PlatformRequestContextHelper
{
    ...
    public static bool TryGetValue<T, TItem>(IDictionary<string, TItem> requestContext, string contextKey, out T item)
    {
        ...
        if (originalValue is Lazy<object?> lazyValue) return HandleLazyValue(lazyValue, out item);
        ...
        if (!originalValue.GetType().IsAssignableTo(typeof(T)))
        {
            ...
            if (originalValue is IEnumerable<object> originalValueListObjectValues &&
                TryGetParsedValuesFromStringValues(out item, originalValueListObjectValues.Where(p => p != null).Select(p => p.ToString()).ToList()))
            {
                ReAssignParsedExactFoundValue(requestContext, item, foundContextKey);
                return true;
            }
            ...
            if (TryGetParsedValuesFromStringValues(out item, originalValue is string originalValueStr ? [originalValueStr] : [originalValue.ToJson()]))
            {
                ReAssignParsedExactFoundValue(requestContext, item, foundContextKey);
                return true;
            }
    ...
    public static bool TryGetParsedValuesFromStringValues<T>(out T foundValue, List<string> stringValues)
    {
        ...
        var isTryGetListValueSuccess = TryGetParsedListValueFromUserClaimStringValues(stringValues, out foundValue);
    ...
    /// <summary>
    /// Handles lazy-loaded values with optimized Task handling
    /// </summary>
    private static bool HandleLazyValue<T>(Lazy<object?> lazyValue, out T item)
    {
        var value = lazyValue.Value;

        // Handle Task<T> scenarios with cached reflection
        if (value is Task<object?> rawTask && IsTaskType<T>()) return HandleTaskType(rawTask, out item);
    ...
    /// <summary>
    /// Handles Task<T> conversion with cached UnboxAsync method
    /// </summary>
    private static bool HandleTaskType<T>(Task<object?> rawTask, out T item)
    {
        try
        {
    ...
    /// </remarks>
    public static bool TryGetParsedListValueFromUserClaimStringValues<T>(List<string> matchedClaimStringValues, out T foundValue)
    {
