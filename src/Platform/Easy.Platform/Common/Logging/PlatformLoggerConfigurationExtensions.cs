...
public static class PlatformLoggerConfigurationExtensions
{
    ...
    public static LoggerConfiguration EnrichDefaultPlatformEnrichers(this LoggerConfiguration loggerConfiguration)
    {
        return loggerConfiguration.Enrich.With(
            // Background thread enricher - captures stack trace before Task.Run to preserve diagnostic context
            // Critical for debugging issues that occur in fire-and-forget background operations
            new PlatformBackgroundThreadFullStackTraceEnricher(),
            // Activity tracing enricher - adds distributed tracing correlation identifiers
            // Enables request tracking across service boundaries and async operation boundaries
    ...
    public static LoggerConfiguration WithExceptionDetails(this LoggerConfiguration loggerConfiguration, Action<DestructuringOptionsBuilder> configDestructurers = null)
    {
        return loggerConfiguration.Enrich.WithExceptionDetails(
            new DestructuringOptionsBuilder()
                // Configure default destructurers for common .NET exception types
                // This provides rich, structured exception information out-of-the-box
                .WithDefaultDestructurers()
                // Conditionally apply custom destructurer configuration if provided
                // Uses platform WithIf extension for clean conditional configuration
