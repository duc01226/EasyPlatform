...
{
    private readonly Dictionary<string, MetadataReaderProvider?> cache = new(StringComparer.Ordinal);
    ...
    {
        if (cache.TryGetValue(assemblyPath, out var provider) && provider is not null)
        {
    ...
    public void Dispose()
    {
        foreach (var entry in cache) entry.Value?.Dispose();
