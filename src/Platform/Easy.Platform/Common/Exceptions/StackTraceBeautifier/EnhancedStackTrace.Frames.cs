...
public partial class EnhancedStackTrace
{
    ...
    public static ResolvedMethod GetMethodDisplayString(MethodBase originMethod)
    {
        ...
        if (method.IsGenericMethod)
        {
            ...
            var genericArgumentsString = string.Join(
                ", ",
                genericArguments
    ...
    private static bool TryResolveGeneratedName(
        ...
        out int? ordinal)
    {
        ...
        var candidateMethods = dt.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly)
        ...
        var candidateConstructors =
            dt.GetConstructors(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                .Where(m => m.Name == matchName);
        ...
        for (var i = 0; i < maxResolveDepth; i++)
        {
            ...
            candidateMethods = dt.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                .Where(m => m.Name == matchName);
            if (TryResolveSourceMethod(candidateMethods, kind, matchHint, ref method, ref type, out ordinal)) return true;
            ...
            if (methodName == ".cctor")
            {
                candidateConstructors = dt.GetConstructors(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.DeclaredOnly)
                    .Where(m => m.Name == matchName);
    ...
    private static bool TryResolveSourceMethod(
        IEnumerable<MethodBase> candidateMethods,
    ...
    private static void GetOrdinal(MethodBase method, ref int? ordinal)
    {
        ...
        if (lamdaStart > 3)
        {
            ...
            var methods = method.DeclaringType?.GetMethods(
        ...
        out int closeBracketOffset)
    {
        ...
        if (name.StartsWith("CS$<", StringComparison.Ordinal))
            openBracketOffset = 3;
        else if (name.StartsWith('<')) openBracketOffset = 0;
    ...
    private static bool ShowInStackTrace(MethodBase method)
    {
        ...
        if (method.Name == "GetResult" && method.DeclaringType?.FullName == "System.Threading.Tasks.Sources.ManualResetValueTaskSourceCore`1") return false;
