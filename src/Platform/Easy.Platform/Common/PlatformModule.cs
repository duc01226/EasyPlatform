...
/// Platform modules are the fundamental building blocks of the BravoSUITE architecture,
...
public interface IPlatformModule
{
    ...
    /// 2. Within each level, modules are ordered by ExecuteInitPriority (descending)
    ...
    /// - Child modules are automatically discovered through ModuleTypeDependencies()
    ...
    /// </remarks>
    public static async Task WaitAllModulesInitiatedAsync(IServiceProvider serviceProvider, Type moduleType, ILogger logger = null, string logSuffix = null, bool notLogging = true)
    {
        if (serviceProvider.GetServices(moduleType).Select(p => p.As<IPlatformModule>()).All(p => p.Initiated))
            return;
...
public abstract class PlatformModule : IPlatformModule, IDisposable
{
    ...
    /// - Used by WaitAllModulesInitiatedAsync for synchronization
    ...
    /// - Coordinated with RegisterServicesExecuted for complete lifecycle tracking
    ///
    ...
    public List<IPlatformModule> AllDependencyChildModules(IServiceCollection useServiceCollection = null, bool includeDeepChildModules = true)
    {
        return ModuleTypeDependencies()
            .Select(moduleTypeProvider =>
            {
                ...
                return includeDeepChildModules ? dependModule.AllDependencyChildModules(useServiceCollection).ConcatSingle(dependModule) : [dependModule];
            })
            .Flatten()
            .ToList();
    }
    ...
    {
        ModuleTypeDependencies().Select(moduleTypeProvider => moduleTypeProvider(Configuration)).ForEach(moduleType => serviceCollection.RegisterModule(moduleType, true));
    }
