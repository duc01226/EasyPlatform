    ...
    where TEvent : IPlatformCqrsEvent
{
    ...
    {
        if (RootServiceProvider.GetService<PlatformModule.DistributedTracingConfig>()?.DistributedTracingStackTraceEnabled() == true && thisHandlerInstanceEvent.StackTrace == null)
    ...
    {
        return NoNeedCloneNewEventInstanceForTheHandler || RootServiceProvider.ImplementationAssignableToServiceTypeRegisteredCount(typeof(IPlatformCqrsEventHandler<TEvent>)) == 1
            ? @event
            : @event
                .DeepClone()
                .PipeAction(_ =>
