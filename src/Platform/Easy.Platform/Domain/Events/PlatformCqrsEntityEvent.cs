...
public static class PlatformCqrsDomainEventsSupportEntityEventExtensions
{
    ...
    public static bool HasAnyFieldUpdatedEvents<TEntity>(this IPlatformCqrsDomainEventsSupportEntityEvent<TEntity> @event, params Expression<Func<TEntity, object>>[] fields)
    {
    ...
    where TEntity : class, IEntity, new()
{
    ...
    public PlatformCqrsEntityEvent(TEntity entityData, PlatformCqrsEntityEventCrudAction crudAction)
    {
        ...
        {
            DomainEvents = businessActionEventsEntity.GetDomainEvents().Select(p => new KeyValuePair<string, string>(p.Key, PlatformJsonSerializer.Serialize(p.Value))).ToList();
        }
    }
