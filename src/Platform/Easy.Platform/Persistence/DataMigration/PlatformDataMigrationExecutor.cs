    ...
    where TDbContext : IPlatformDbContext
{
        ...
        IServiceProvider serviceProvider)
    {
        ...
        allDataMigrationExecutors.ForEach(dataMigrationExecutor =>
        {
    ...
    {
        var dbInitializedMigrationHistory = await dbContext.FirstAsync(dbContext.DataMigrationHistoryQuery().Where(p => p.Name == dbInitializedMigrationHistoryName));
