...
public interface IPlatformPersistenceModule : IPlatformModule
{
    ...
    ///
    /// The method respects <see cref="ForCrossDbMigrationOnly"/> and <see cref="DisableDbInitializingAndMigration"/>
    ...
    /// 2. Ordering modules by their <see cref="IPlatformModule.ExecuteInitPriority"/> (descending)
    ...
    /// </remarks>
    public static async Task ExecuteDependencyPersistenceModuleMigrateApplicationData(List<Type> moduleTypeDependencies, IServiceProvider serviceProvider)
    {
...
public abstract class PlatformPersistenceModule : PlatformModule, IPlatformPersistenceModule
{
    ...
    ///
    /// Used by <see cref="InitializeDb"/> and <see cref="MigrateDataAsync"/> for handling
    ...
    /// The base class coordinates dependency migration and sets <see cref="MigrateDataExecuted"/> upon completion.
    ///
    /// Called during application startup after <see cref="InitializeDb"/> completes successfully.
    /// </remarks>
    ...
    ///
    /// **Service Discovery:**
    /// Uses assembly scanning from <see cref="GetServicesRegisterScanAssemblies"/> to automatically
    ...
    /// - <see cref="EnableInboxBusMessage"/> returns <c>true</c>
    ...
    ///
    /// Registration occurs only when:
    /// - <see cref="EnableOutboxBusMessage"/> returns <c>true</c>
    /// - Module is not in <see cref="ForCrossDbMigrationOnly"/> mode
    ///
    ...
    private void RegisterRepositories(IServiceCollection serviceCollection)
    {
        ...
        if (RegisterLimitedRepositoryImplementationTypes()?.Any() == true)
        {
            ...
            RegisterLimitedRepositoryImplementationTypes().ForEach(repositoryImplementationType => serviceCollection.RegisterAllForImplementation(repositoryImplementationType));
        }
        else
        {
    ...
    where TDbContext : class, IPlatformDbContext<TDbContext>
{
    ...
    /// - <see cref="MigrateDataExecuted"/>: Database migration completed
    ///
    ...
    /// - Skipped if <see cref="ForCrossDbMigrationOnly"/> or <see cref="DisableDbInitializingAndMigration"/> is true
    ///
    /// **3. State Management:**
    /// - Sets <see cref="MigrateDataExecuted"/> to true upon successful completion
    /// - Enables the <see cref="Initiated"/> property to return true when combined with base initialization
    ///
    /// **Retry Logic:**
    ...
    /// - Skipped when <see cref="ForCrossDbMigrationOnly"/> is true (cross-database access only)
    /// - Skipped when <see cref="DisableDbInitializingAndMigration"/> is true (shared database scenarios)
    ///
    ...
    public override async Task InitializeDb(IServiceScope serviceScope)
    {
        ...
        await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(
            ...
            DefaultDbInitAndMigrationRetryCount,
            exception =>
                Logger.LogError(exception.BeautifyStackTrace(), "[{DbContext}] {ExceptionType} detected on attempt InitializeDb", typeof(TDbContext).Name, exception.GetType().Name)
        );
    }
    ...
    /// - Applies pooling options from <see cref="PooledDbContextOption"/>
    ...
    protected override void InternalRegister(IServiceCollection serviceCollection)
    {
        ...
        if (PooledDbContextOption().Enabled)
    ...
    /// {
    ///     var poolOptions = PooledDbContextOption();
