...
public class PlatformFileMimeTypeMapper
{
    ...
    private static readonly Lazy<PlatformFileMimeTypeMapper> LazyInstance = new(() => new PlatformFileMimeTypeMapper());
    ...
    /// </summary>
    protected PlatformFileMimeTypeMapper()
        : this(
    ...
    /// </summary>
    public PlatformFileMimeTypeMapper(Dictionary<string, string> fileExtensionToMimeTypeMaps)
    {
        FileExtensionToMimeTypeMaps = fileExtensionToMimeTypeMaps ?? throw new ArgumentNullException(nameof(fileExtensionToMimeTypeMaps));
        MimeTypeToFileExtensionMaps = fileExtensionToMimeTypeMaps.GroupBy(p => p.Value, p => p.Key).ToDictionary(p => p.Key, p => p.ToArray());
    }

    public static PlatformFileMimeTypeMapper Instance => LazyInstance.Value;

    /// <summary>
    /// The cross reference table of file extensions and mime-types.
    /// </summary>
    public Dictionary<string, string> FileExtensionToMimeTypeMaps { get; }

    /// <summary>
    /// The cross reference table of file extensions and mime-types.
    /// </summary>
    public Dictionary<string, string[]> MimeTypeToFileExtensionMaps { get; }
    ...
    {
        var extension = GetFileExtensionByFilePath(filePath);

        return extension == null ? null : FileExtensionToMimeTypeMaps.GetValueOrDefault(extension, UnknownBinary);
    }

    /// <summary>
    /// Given mimeType type, determine the file extension
    /// </summary>
    public string[] GetFileExtensions(string mimeType)
    {
        return MimeTypeToFileExtensionMaps.GetValueOrDefault(mimeType);
    }

    private static string GetFileExtensionByFilePath(string filePath)
    {
