    ...
    where TDbContext : PlatformEfCoreDbContext<TDbContext>, IPlatformDbContext<TDbContext>
{
    ...
    public virtual async Task Initialize(IServiceProvider serviceProvider)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new Exception(
                $"{GetType().Name} Initialize failed.",
                ex);
        }
        ...
        where TEntity : class, IEntity, new()
    {
        ...
        if (isEntityTracked && checkEntityModifiedByExistingEntity != null)
        {
            var changedMutableFields = entity.GetChangedFields(
                ...
                .PipeAction(changedMutableFields => changedMutableFields?.ForEach(p => Entry(entity!).Property(p.Key).IsModified = true));

            if (isEntityTrackedModified == false)
            {
                isEntityTrackedModified = changedMutableFields?.Any() == true ||
