    ...
    where TDbContext : PlatformEfCoreDbContext<TDbContext>
{
    ...
    public override IQueryable<TEntity> GetQuery(IPlatformUnitOfWork uow, Expression<Func<TEntity, object?>>[] loadRelatedEntities, bool forAsyncEnumerable)
    {
        ...
        return GetTable(uow)
            ...
            .PipeIf(loadRelatedEntities.Any(), query => loadRelatedEntities.Aggregate(query, (query, loadRelatedEntityFn) => query.Include(loadRelatedEntityFn).DefaultIfEmpty()));
    }
    ...
    protected override void HandleDisposeUsingOnceTransientUowLogic<TResult>(IPlatformUnitOfWork uow, Expression<Func<TEntity, object>>[] loadRelatedEntities, TResult result)
    {
