...
public class PlatformAspNetApplicationRequestContext : IPlatformApplicationRequestContext
{
    ...
    protected List<string> GetAllKeys(HttpContext? useHttpContext, bool includeIgnoredKeys = false)
    {
        ...
        var userClaimsTypeKeys = Util.TaskRunner.CatchException<ObjectDisposedException, IEnumerable<string>>(() => useHttpContext?.User.Claims.Select(p => p.Type) ?? [], []);
        var requestHeadersKeys = Util.TaskRunner.CatchException<ObjectDisposedException, IEnumerable<string>>(() => useHttpContext?.Request.Headers.Select(p => p.Key) ?? [], []);
