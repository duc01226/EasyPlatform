...
export class AppTextSnippetDetailComponent extends PlatformVmComponent<AppTextSnippetDetail> implements OnInit {
    ...
    public loadSelectedTextSnippetItem = this.effectSimple((_, isReloading?: boolean) => {
    ...
    }, 'loadSelectedTextSnippetItem');

    public onSaveSelectedTextSnippetItem = this.effectSimple((_, isReloading?: boolean) => {
        this.updateVm({ saveTextSnippetError: null });

        return this.snippetTextApi.save(new SaveTextSnippetCommand({ data: this.currentVm().toSaveTextSnippet })).pipe(
            this.tapResponse(
                result => {
                    if (this.currentVm().isCreateNew()) {
                        ...
                        if (this.appStore.currentState().selectedSnippetTextId != this.currentVm().toSaveTextSnippetId) {
    ...
    public override initOrReloadVm = (isReload: boolean): Observable<AppTextSnippetDetail> => {
        return of(new AppTextSnippetDetail({ toSaveTextSnippetId: this.appStore.currentState().selectedSnippetTextId }));
    };
}
