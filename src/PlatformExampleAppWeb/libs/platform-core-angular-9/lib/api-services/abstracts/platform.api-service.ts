...
import { HttpStatusCode, IPlatformApiServiceErrorResponse, PlatformApiServiceErrorInfoCode, PlatformApiServiceErrorResponse } from './platform.api-error';
...
export abstract class PlatformApiService extends PlatformHttpService {
    ...
    protected get defaultOptions(): HttpClientOptions {
        const defaultOptions = PlatformHttpService.DefaultOptions();
    ...
    protected put<T>(path: string, body: T, configureOptions?: (option: HttpClientOptions) => HttpClientOptions | void | undefined): Observable<T> {
        ...
        return super
            .httpPut<T>(this.apiUrl + path, <T>(<unknown>this.preprocessData(body)), configuredOptions ?? options)
            .pipe(catchError(err => this.catchHttpError<T>(err, path, body)));
    }
    ...
    private preprocessData<T>(data: T): IApiGetParams | Record<string, string> | FormData {
        ...
        return toPlainObj(removeNullProps(<object>(<unknown>data)));
    }
    ...
    private parseHttpGetParam(inputParams: IApiGetParams | Record<string, string> | FormData): HttpParams {
        ...
        for (const paramKey in flattenedInputParams) {
            if (Object.getOwnPropertyDescriptor(flattenedInputParams, paramKey) != null) {
