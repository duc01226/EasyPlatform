...
export class PlatformApiServiceErrorResponse {
    ...
    public static getDefaultFormattedMessage(errorResponse: PlatformApiServiceErrorResponse | Error): string {
        if (errorResponse instanceof Error) {
            const errorMessage = `[ERROR] Something went wrong, app has been crashed.\n\n${errorResponse.stack}`;
            return pipe(errorMessage, [pipeAction<string>(PlatformApiServiceErrorResponse.showErrorAlert)]);
        } else {
            return errorResponse.getDefaultFormattedMessage();
        }
    }

    /**
     * Helper method for showing error alerts - used to avoid lambda functions in metadata.
     * @param errorMessage - The error message to display in alert
     */
    private static showErrorAlert(errorMessage: string): string {
        alert(errorMessage);
        return errorMessage;
    }
