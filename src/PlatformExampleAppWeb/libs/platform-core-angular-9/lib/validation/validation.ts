...
export class Validation<TValue = void> {
    ...
    public static validate<TValue>(value: TValue, validCondition: boolean, error: ValidationError | string): Validation<TValue> {
        return validCondition ? Validation.valid(value) : Validation.invalid(value, error);
    }

    public static validateNot<TValue>(value: TValue, invalidCondition: boolean, error: ValidationError | string): Validation<TValue> {
        return !invalidCondition ? Validation.valid(value) : Validation.invalid(value, error);
    }

    public and(validCondition: (value: TValue | undefined) => boolean, error: ValidationError | string): Validation<TValue> {
        if (!this.isValid) return this;

        return <Validation<TValue>>(validCondition(this.value) ? Validation.valid(this.value) : Validation.invalid(this.value, error));
    }

    public andNot(invalidCondition: (value: TValue | undefined) => boolean, error: ValidationError | string): Validation<TValue> {
        if (!this.isValid) return this;

        return <Validation<TValue>>(!invalidCondition(this.value) ? Validation.valid(this.value) : Validation.invalid(this.value, error));
    }

    public andNextValidate<TNextValue>(nextValidate: (value: TValue | undefined) => Validation<TNextValue>): Validation<TNextValue> {
