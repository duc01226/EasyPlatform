...
import { catchError, delay, filter, map, shareReplay, switchMap, take, takeUntil, tap, throttleTime } from 'rxjs/operators';
...
export abstract class PlatformVmStore<TViewModel extends PlatformVm> implements OnDestroy {
    ...
    /**
     * Cache for error message BehaviorSubjects to avoid recreation.
     */
    private cachedErrorMsg$: Dictionary<BehaviorSubject<string | undefined>> = {};
    ...
    /**
     * Cache for loading state BehaviorSubjects to avoid recreation.
     */
    private cachedLoading$: Dictionary<BehaviorSubject<boolean | undefined>> = {};

    /**
     * Cache for reloading state BehaviorSubjects to avoid recreation.
     */
    private cachedReloading$: Dictionary<BehaviorSubject<boolean | undefined>> = {};
     ...
     *
     * @param defaultState - Initial state for the store
     *
     * @example
     * ```typescript
     * constructor() {
     ...
     * ```
     */
    constructor(defaultState: TViewModel, public cacheService: PlatformCachingService) {
     ...
     *
     * @returns The ComponentStore instance
     */
     ...
     *
     * @returns The view model observable stream
     *
     * @example
     * ```typescript
    ...
    /**
     * The view model as a BehaviorSubject.
     * Provides reactive access to the current view model state.
     */
    private _vm?: BehaviorSubject<TViewModel | undefined>;

    /**
     * Gets the view model as a BehaviorSubject.
     *
     * Provides reactive access to the view model state using Angular's BehaviorSubject system.
     * Automatically updates when the underlying observable emits new values.
     *
     * @returns BehaviorSubject containing the view model
     *
     * @example
     * ```typescript
     * // Use in component
     ...
     *
     *   get data() {
     *     const vm = this.store.vm.getValue();
     *     return vm?.data ?? [];
     *   }
     *
     *   get isLoading() {
     *     return this.store.vm.getValue()?.isStateLoading ?? false;
     *   }
     * }
     * ```
     */
    public get vm(): BehaviorSubject<TViewModel | undefined> {
        if (this._vm == undefined) {
            this._vm = rxjs_toBehaviorSubject(this.vm$, this.defaultState);
        }
     ...
     *
     * @abstract
     * @param [data] - Optional initial data for the view model
     ...
     *
     * @abstract
     * @param isReload - Whether this is a reload operation (true) or initial load (false)
     * @returns Observable representing the data loading operation
     *
     * @example
     * ```typescript
     * public initOrReloadVm = (isReload: boolean) => {
     ...
     *
     * @returns Observable that completes when reload is finished
     *
     * @example
     * ```typescript
    ...
    private _isStatePending?: BehaviorSubject<boolean>;
    public get isStatePending(): BehaviorSubject<boolean> {
        if (this._isStatePending == null) {
            this._isStatePending = rxjs_toBehaviorSubject(
    ...
    private _isStateLoading?: BehaviorSubject<boolean>;
    public get isStateLoading(): BehaviorSubject<boolean> {
        if (this._isStateLoading == null) {
            this._isStateLoading = rxjs_toBehaviorSubject(
    ...
    private _isStateReloading?: BehaviorSubject<boolean>;
    public get isStateReloading(): BehaviorSubject<boolean> {
        if (this._isStateReloading == null) {
            this._isStateReloading = rxjs_toBehaviorSubject(
    ...
    private _isStateSuccess?: BehaviorSubject<boolean>;
    public get isStateSuccess(): BehaviorSubject<boolean> {
        if (this._isStateSuccess == null) {
            this._isStateSuccess = rxjs_toBehaviorSubject(
    ...
    private _isStateSuccessOrReloading?: BehaviorSubject<boolean>;
    public get isStateSuccessOrReloading(): BehaviorSubject<boolean> {
        if (this._isStateSuccessOrReloading == null) {
            this._isStateSuccessOrReloading = rxjs_toBehaviorSubject(
    ...
    private _isStateError?: BehaviorSubject<boolean>;
    public get isStateError(): BehaviorSubject<boolean> {
        if (this._isStateError == null) {
            this._isStateError = rxjs_toBehaviorSubject(
    ...
    public get isStatePending$(): Observable<boolean> {
        if (this._isStatePending$ == null) this._isStatePending$ = this.internalSelect(_ => _.isStatePending);
    ...
    public get isStateSuccess$(): Observable<boolean> {
        if (this._isStateSuccess$ == null) this._isStateSuccess$ = this.internalSelect(_ => _.isStateSuccess);
    ...
    public get isStateSuccessOrReloading$(): Observable<boolean> {
        if (this._isStateSuccessOrReloading$ == null) this._isStateSuccessOrReloading$ = this.internalSelect(_ => _.isStateSuccess || _.isStateReloading);
    ...
    public get isStateError$(): Observable<boolean> {
        if (this._isStateError$ == null) this._isStateError$ = this.internalSelect(_ => _.isStateError);
    ...
    /**
     * Returns the error message BehaviorSubject for a specific request key.
     *
     * @param requestKey - Key to identify the request.
     * @returns Error message BehaviorSubject.
     */
    public getErrorMsg$ = (requestKey?: string) => {
        ...
        if (this.cachedErrorMsg$[requestKey] == null) {
            this.cachedErrorMsg$[requestKey] = rxjs_toBehaviorSubject(this.getErrorMsgObservable$(requestKey), undefined);
        }
    ...
    /**
     * Returns the loading state BehaviorSubject for a specific request key.
     *
     * @param requestKey - Key to identify the request.
     * @returns Loading state BehaviorSubject.
     */
    public isLoading$ = (requestKey: string = requestStateDefaultKey) => {
        if (this.cachedLoading$[requestKey] == null) {
            this.cachedLoading$[requestKey] = rxjs_toBehaviorSubject(
                this.internalSelect(_ => _.isLoading(requestKey)),
                false
            );
        }
    ...
    /**
     * Returns the reloading state BehaviorSubject for a specific request key.
     *
     * @param requestKey - Key to identify the request.
     * @returns Reloading state BehaviorSubject.
     */
    public isReloading$ = (requestKey: string = requestStateDefaultKey) => {
        if (this.cachedReloading$[requestKey] == null) {
            this.cachedReloading$[requestKey] = rxjs_toBehaviorSubject(
                this.internalSelect(_ => _.isReloading(requestKey)),
                false
            );
        }
     ...
     *
     * @param generator *   The generator function that defines the effect logic. Receives:
     ...
     *
     * @method
     * @returns The cached view model state or undefined if not found.
     */
