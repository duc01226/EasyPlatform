 ...
 * @param validatorFn - The async validation function
 * @param [throttleTimeMs=500] - Throttle delay in milliseconds to prevent excessive API calls
 * @param [onlyThrottleValidForm=false] - If true, only throttle when the form is valid
 * @returns A configured Angular async validator function
 *
 * @example
 * ```typescript
