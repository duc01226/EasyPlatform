 ...
 */
export abstract class PlatformVmStoreComponent<TViewModel extends PlatformVm, TViewModelStore extends PlatformVmStore<TViewModel>> extends PlatformComponent
    implements OnInit {
    /**
     ...
     *
     * @param store - The main store instance for this component
     *
     * @example
     * ```typescript
     * export class UserProfileComponent extends PlatformVmStoreComponent<UserProfileVm, UserProfileStore> {
     ...
     *
     * @returns Array of additional store instances found on this component
     *
     * @example
     * ```typescript
     * export class DashboardComponent extends PlatformVmStoreComponent<DashboardVm, DashboardStore> {
    ...
    public get additionalStores(): PlatformVmStore<PlatformVm>[] {
        if (this._additionalStores == null) {
            ...
            this._additionalStores = keys(this, true, [
                ...
                .map(key => <PlatformVmStore<PlatformVm>>(<unknown>this[key]));
        }
     ...
     *
     * @returns Observable stream of the complete state
     *
     * @example
     * ```typescript
     * ngOnInit() {
     ...
     * - **Lifecycle Aware**: Automatically cleaned up when component is destroyed
     *
     * @returns Observable stream of valid view model data
     *
     * @example
     * ```typescript
    ...
    /**
     * Internal BehaviorSubject cache for the view model.
     */
    private _vm?: BehaviorSubject<TViewModel | undefined>;
     ...
     *
     * @returns Signal containing the current view model or undefined
     *
     * @example
     * ```typescript
     * // Template usage (recommended)
     ...
     * ```
     */
    public get vm(): BehaviorSubject<TViewModel | undefined> {
        if (this._vm == undefined) {
            this._vm = this.store.vm;

            // Init all additionalStores BehaviorSubject by accessing them
     ...
     *
     * @returns Signal that emits true when the main store state is pending
     *
     * @example
     * ```typescript
     * // Template usage
     ...
     *
     * @returns Signal that emits true when any store is loading
     *
     * @example
     * ```typescript
     * // Template usage
    ...
    public get isStateLoading(): BehaviorSubject<boolean> {
        if (this._isStateLoading == null) {
            const loadingObservables = this.additionalStores.concat([<PlatformVmStore<PlatformVm>>(<unknown>this.store)]).map(store => store.isStateLoading$);
            this._isStateLoading = new BehaviorSubject<boolean>(false);
            combineLatest(loadingObservables)
                .pipe(
                    this.untilDestroyed(),
                    map(isLoadings => isLoadings.find(isLoading => isLoading) != undefined)
                )
                .subscribe(result => this._isStateLoading!.next(result));
        }
     ...
     * The signal is created lazily and cached for performance.
     *
     * @returns Signal that emits true when any store is reloading
     *
     * @example
     * ```typescript
     * // Template usage
    ...
    public get isStateReloading(): BehaviorSubject<boolean> {
        if (this._isStateReloading == null) {
            const reloadingObservables = this.additionalStores
                .concat([<PlatformVmStore<PlatformVm>>(<unknown>this.store)])
                .map(store => store.isStateReloading$);
            this._isStateReloading = new BehaviorSubject<boolean>(false);
            combineLatest(reloadingObservables)
                .pipe(
                    this.untilDestroyed(),
                    map(isReloadings => isReloadings.find(isReloading => isReloading) != undefined)
                )
                .subscribe(result => this._isStateReloading!.next(result));
        }
     ...
     *
     * @returns Signal that emits true when loading and no view model is available
     *
     * @example
     * ```typescript
    ...
    public get isLoadingToInitVm(): BehaviorSubject<boolean> {
        if (this._isLoadingToInitVm == null) {
            this._isLoadingToInitVm = new BehaviorSubject<boolean>(false);
            const updateIsLoadingToInitVm = () => {
                const isLoading = (this.isStateLoading.getValue() || this.isStateReloading.getValue()) && this.vm.getValue() == undefined;
                this._isLoadingToInitVm!.next(isLoading);
            };
            this.isStateLoading.pipe(this.untilDestroyed()).subscribe(() => updateIsLoadingToInitVm());
            this.isStateReloading.pipe(this.untilDestroyed()).subscribe(() => updateIsLoadingToInitVm());
            this.vm.pipe(this.untilDestroyed()).subscribe(() => updateIsLoadingToInitVm());
        }
     ...
     *
     * @returns Signal that emits true when the main store state is successful
     *
     * @example
     * ```typescript
     * // Template usage
     ...
     * The signal is created lazily and cached for performance.
     *
     * @returns Signal that emits true when any store is in an error state
     *
     * @example
     * ```typescript
     * // Template usage
    ...
    public get isStateError(): BehaviorSubject<boolean> {
        if (this._isStateError == null) {
            const errorObservables = this.additionalStores.concat([<PlatformVmStore<PlatformVm>>(<unknown>this.store)]).map(store => store.isStateError$);
            this._isStateError = new BehaviorSubject<boolean>(false);
            combineLatest(errorObservables)
                .pipe(
                    this.untilDestroyed(),
                    map(isErrors => isErrors.find(isError => isError) == true)
                )
                .subscribe(result => this._isStateError!.next(result));
        }
        return this._isStateError!;
    }

    protected initOrReloadVm: (isReload: boolean) => Observable<TViewModel | undefined> | undefined = (isReload: boolean) => {
        return undefined;
    };
     ...
     * });
     *
     * @param requestKey - The request key for which error messages are retrieved. Defaults to the default request state key.
     * @returns - A Signal observable emitting the error message associated with the specified request key.
     *
     * @method
     * @override
     ...
     */
    public getErrorMsg$(requestKey: string = requestStateDefaultKey): BehaviorSubject<string | undefined> {
        if (this.cachedErrorMsg$[requestKey] == null) {
            const errorObservables = this.additionalStores
                .concat([<PlatformVmStore<PlatformVm>>(<unknown>this.store)])
                .map(store => store.getErrorMsgObservable$(requestKey));
            this.cachedErrorMsg$[requestKey] = new BehaviorSubject<string | undefined>(undefined);
            combineLatest(errorObservables)
                .pipe(
                    this.untilDestroyed(),
                    map(errors => errors.find(p => p != null))
                )
                .subscribe(result => this.cachedErrorMsg$[requestKey]!.next(result));
        }
     ...
     *
     * @param [requestKeys] - Optional array of request keys to include. If provided, only errors from these keys are included
     * @param [excludeKeys] - Optional array of request keys to exclude from the result
     * @returns Signal emitting combined error messages or undefined if no errors
     *
     * @example
     * ```typescript
    ...
    public getAllErrorMsgs$(requestKeys?: string[], excludeKeys?: string[]): BehaviorSubject<string | undefined> {
        ...
        if (this.cachedAllErrorMsgs$[combinedCacheRequestKey] == null) {
            ...
            this.cachedAllErrorMsgs$[combinedCacheRequestKey] = new BehaviorSubject<string | undefined>(undefined);
            combineLatest(errorObservables)
                .pipe(
                    this.untilDestroyed(),
                    map(errors => list_distinct(errors.filter(msg => msg != null && msg.trim() != '')).join('; '))
                )
                .subscribe(result => this.cachedAllErrorMsgs$[combinedCacheRequestKey]!.next(result));
        }
     ...
     * @param [options.updaterNotDeepMutate] - Whether to prevent deep mutation in updater functions
     * @param [options.assignDeepLevel] - The depth level for deep assignment operations
     *
     * @example
     * ```typescript
     * // Update user profile
     ...
     *
     * @param [requestKey=requestStateDefaultKey] - The request key to check loading state for
     * @returns Signal that emits true when any store is loading for the request key
     *
     * @example
     * ```typescript
    ...
    public isLoading$(requestKey: string = requestStateDefaultKey): BehaviorSubject<boolean | null> {
        if (this.cachedLoading$[requestKey] == null) {
            this.cachedLoading$[requestKey] = new BehaviorSubject<boolean | null>(null);
            const updateLoading = () => {
                const isLoading =
                    this.store.isLoading$(requestKey).getValue() || this.additionalStores.find(s => s.isLoading$(requestKey).getValue()) != undefined;
                this.cachedLoading$[requestKey]!.next(isLoading);
            };
            this.store
                .isLoading$(requestKey)
                .pipe(this.untilDestroyed())
                .subscribe(() => updateLoading());
            this.additionalStores.forEach(s =>
                s
                    .isLoading$(requestKey)
                    .pipe(this.untilDestroyed())
                    .subscribe(() => updateLoading())
     ...
     *
     * @param [requestKey=requestStateDefaultKey] - The request key to check reloading state for
     * @returns Signal that emits true when any store is reloading for the request key
     *
     * @example
     * ```typescript
    ...
    public isReloading$(requestKey: string = requestStateDefaultKey): BehaviorSubject<boolean | null> {
        if (this.cachedReloading$[requestKey] == null) {
            this.cachedReloading$[requestKey] = new BehaviorSubject<boolean | null>(null);
            const updateReloading = () => {
                const isReloading =
                    this.store.isReloading$(requestKey).getValue() || this.additionalStores.find(s => s.isReloading$(requestKey).getValue()) != undefined;
                this.cachedReloading$[requestKey]!.next(isReloading);
            };
            this.store.isReloading$(requestKey).subscribe(() => updateReloading());
            this.additionalStores.forEach(s => s.isReloading$(requestKey).subscribe(() => updateReloading()));
        }
     ...
     * @param [requestKey] - Optional request key to associate with the loading/error state
     * @param [options] - Optional configuration for the observer
     * @returns RxJS operator function for state management
     *
     * @example
     * ```typescript
