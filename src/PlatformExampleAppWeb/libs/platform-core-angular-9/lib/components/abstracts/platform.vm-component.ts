...
import { rxjs_combineLatestBehaviorSubject } from '../../rxjs/rxjs-to-behavior-subject';
 ...
 *   template: `<div *ngIf="vm | async as viewModel">{{viewModel.name}}</div>`
 * })
...
export abstract class PlatformVmComponent<TViewModel extends IPlatformVm> extends PlatformComponent implements OnInit {
    ...
    /**
     * Internal BehaviorSubject storage for the view model.
    ...
    /**
     * Gets the view model as a BehaviorSubject for reactive programming.
     *
     * The BehaviorSubject is created lazily on first access and provides reactive updates
     ...
     * <div *ngIf="vm.getValue() as viewModel">
     ...
     *     this.isUserAdmin = vm?.user?.role === 'admin';
     *   });
     * }
     *
     * onSave() {
     *   const currentVm = this.vm.getValue();
     ...
     * ```
     *
     * @returns A BehaviorSubject containing the current view model or undefined
     *
     * @see {@link https://rxjs.dev/guide/subject#behaviorsubject} RxJS BehaviorSubject Guide
     */
    public get vm(): BehaviorSubject<TViewModel | undefined> {
        if (this._vm$ == null) this._vm$ = new BehaviorSubject<TViewModel | undefined>(this._vm);
        return this._vm$;
    }
     ...
     * in favor of using the `vm | async` pipe or the `currentVm()` method for type safety.
     *
     */
     ...
     *
     */
    private _vmObservable$?: Observable<TViewModel>;
    ...
    public get vm$(): Observable<TViewModel> {
        if (this._vmObservable$ == null) this._vmObservable$ = <Observable<TViewModel>>this.vm.pipe(
                filter(p => p != undefined),
                this.untilDestroyed(),
                share()
            );

        return this._vmObservable$;
    }
    ...
    /**
     * BehaviorSubject indicating whether the component is loading to initialize the view model.
     *
     * This BehaviorSubject provides reactive access to the loading state specifically during
     ...
     * ```html
     * <div *ngIf="isLoadingToInitVm | async; else content">
     ...
     * ```typescript
     * ngOnInit() {
     *   this.isLoadingToInitVm.subscribe(isLoading => {
     *     if (isLoading) {
     ...
     * ```html
     * <button
     *   [disabled]="isLoadingToInitVm | async"
     *   (click)="save()">
     *   {{(isLoadingToInitVm | async) ? 'Loading...' : 'Save'}}
     * </button>
     * ```
     *
     * @returns BehaviorSubject<boolean> - Reactive loading state for VM initialization
     *
    ...
    public get isLoadingToInitVm(): BehaviorSubject<boolean> {
        if (this._isLoadingToInitVm == null) {
            this._isLoadingToInitVm = rxjs_combineLatestBehaviorSubject(
                this.isStateLoading,
                this.isStateReloading,
                (isStateLoading, isStateReloading) => (isStateLoading || isStateReloading) && this.vm.getValue() == undefined
            );
        }
    ...
    protected get autoRunInitOrReloadVmInNgOnInit() {
        return false;
    }
    ...
    public initVm(forceReinit: boolean = false, onSuccess?: () => unknown) {
        ...
        if ((this.vm.getValue() == undefined || forceReinit) && initialVm$ != undefined) {
    ...
    protected destroyAllSubjects(): void {
        ...
        if (this._vm$ != null) this.vm.complete();
    }
}
