...
import { PlatformLoadingErrorIndicatorComponent } from './components/platform-loading-error-indicator/platform-loading-error-indicator.component';
import { PlatformTranslatedToastComponent } from './components/toast.component';
import { DisabledControlDirective, SwipeToScrollDirective } from './directives';
...
import { LogTimesDisplayPipe, PlatformHighlightSearchTextPipe, PlatformPipe } from './pipes';
 ...
 */
@NgModule({
    imports: [CommonModule, TranslateModule, MatProgressBarModule, MatIconModule, MatButtonModule],
    declarations: [
        ...
        PlatformLoadingErrorIndicatorComponent
    ],
    exports: [
        CommonModule,
        FormsModule,
...
export class PlatformCoreModule {
    ...
    public static forRoot<TModuleConfig extends PlatformCoreModuleConfig = PlatformCoreModuleConfig>(config: {
    ...
    }): ModuleWithProviders<ForRootAggregatedPlatformCoreModule> {
        // Extract TranslateModule providers
        ...
        // Extract ToastrModule providers
        const toastrModuleWithProviders = ToastrModule.forRoot(
            Object.assign(
                {
                    newestOnTop: true,
                    positionClass: 'toast-bottom-right',
        ...
        return {
            ngModule: ForRootAggregatedPlatformCoreModule,
            providers: [
                // PlatformCoreModule providers
                    ...
                    : [
                          {
                              ...
                              useFactory: PlatformCoreModule.defaultConfigFactory
                          }
                      ]),
                ...
                ...(config.eventHandlerMaps != null ? list_selectMany(config.eventHandlerMaps, PlatformCoreModule.extractEventHandlers) : []),
                {
                    provide: PlatformEventManager,
                    useClass: config.eventManager ?? PlatformEventManager
                },

                ...PlatformCoreModule.buildCanBeInChildModuleProviders({
                ...
                },
                {
                    ...
                    useFactory: config.cachingServiceFactory != null ? config.cachingServiceFactory : PlatformCoreModule.defaultCachingServiceFactory
                },
                ...
                // ToastrModule providers
                ...(toastrModuleWithProviders.providers || [])
            ]
        };
    }

    /**
     * Legacy method that returns array of ModuleWithProviders for backward compatibility.
     * Use forRoot() for new implementations which returns a single aggregated module.
     */
    public static forRootArray<TModuleConfig extends PlatformCoreModuleConfig = PlatformCoreModuleConfig>(config: {
        moduleConfig?: {
            type: Type<TModuleConfig>;
            configFactory: () => TModuleConfig;
        };
    ...
    public static forChild(config: {
        apiServices?: Type<PlatformApiService>[];
        httpOptionsConfigService?: Type<PlatformHttpOptionsConfigService>;
    }): ModuleWithProviders<PlatformCoreModule> {
        return {
            ngModule: PlatformCoreModule,
            providers: [
                ...PlatformCoreModule.buildCanBeInChildModuleProviders({
    ...
    /**
     * Default factory method for creating the caching service.
     */
    private static defaultCachingServiceFactory(): PlatformCachingService {
        return new PlatformCacheStorageCachingService();
    }

    /**
     * Default factory method for creating the module config.
     */
    private static defaultConfigFactory(): PlatformCoreModuleConfig {
        return new PlatformCoreModuleConfig();
    }

    /**
     * Extracts event handlers from event handler map entries.
     */
    private static extractEventHandlers([event, eventHandlers]: [any, any[]]): any[] {
        return eventHandlers;
    }
}

/**
 * Aggregated module that includes all dependencies and providers
 */
@NgModule({
    imports: [
        CommonModule,
...
export class ForRootAggregatedPlatformCoreModule {}
