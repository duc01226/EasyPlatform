...
export class PlatformCacheStorageCachingService extends PlatformCachingService {
     ...
     */
    public set<T>(key: string, data: T | undefined, options?: PlatformCachingServiceSetCacheOptions): void {
        if (data == undefined) this.delete(key);
        else this.doSetData<T>(<NonNullable<T>>(<unknown>data), options, key);
    }

    private doSetData<T>(data: NonNullable<T>, options: PlatformCachingServiceSetCacheOptions | undefined, key: string) {
        const serializedData = JSON.stringify(toPlainObj(<object>(<unknown>data)));
