...
export abstract class PlatformApiService extends PlatformHttpService {
     ...
     */
    private getFromServer<T>(path: string, params: unknown, configureOptions: ((option: HttpClientOptions) => HttpClientOptions | void | undefined) | undefined) {
    ...
    /**
     * Performs HTTP POST request with optional caching support.
     *
     * @remarks
     * POST requests are typically not cached since they modify server state,
     * but caching can be enabled for expensive query operations that use POST
     * for complex request payloads.
     *
     * **When to Enable Caching:**
     * - Complex search queries with large filter objects
     * - Report generation with extensive parameters
     * - Read-only operations that use POST for payload size reasons
     *
     * @param path - API endpoint path relative to `apiUrl`
     * @param body - Request payload to send in the body
     * @param configureOptions - Optional function to customize HTTP options
     * @param enableCache - Whether to cache the response (default: false)
     * @returns Observable stream of the response data
     *
     * @example
     ...
     *
     * @example
     * **Cached POST for complex search:**
     * ```typescript
     * searchProducts(complexCriteria: ProductSearchCriteria): Observable<Product[]> {
     *   // Enable caching for expensive search operations
     *   return this.post<Product[]>('/products/search', complexCriteria, undefined, true);
     * }
     * ```
     *
     * @example
     ...
     *   return this.post<SubmissionResult>('/forms/submit', formData, (options) => {
     ...
     * @returns Observable stream of the server response
     */
    private postFromServer<T>(path: string, body: unknown, configureOptions?: (option: HttpClientOptions) => HttpClientOptions | void | undefined): Observable<T> {
        const options = this.getHttpOptions();
        const configuredOptions = configureOptions != null ? <HttpClientOptions | undefined>configureOptions(options) : options;

        return super.httpPost<T>(this.apiUrl + path, this.preprocessData(body), configuredOptions ?? options).pipe(catchError(err => this.catchHttpError<T>(err, path, body)));
    }
     ...
     * ```
     */
    protected postFileMultiPartForm<T>(path: string, body: unknown, configureOptions?: (option: HttpClientOptions) => HttpClientOptions | void | undefined): Observable<T> {
        const options = this.getHttpOptions();
        const configuredOptions = configureOptions != null ? <HttpClientOptions | undefined>configureOptions(options) : options;

        return super.httpPostFileMultiPartForm<T>(this.apiUrl + path, <object>body, configuredOptions ?? options).pipe(catchError(err => this.catchHttpError<T>(err, path, body)));
    }
    ...
    protected put<T>(path: string, body: T, configureOptions?: (option: HttpClientOptions) => HttpClientOptions | void | undefined): Observable<T> {
        ...
        return super.httpPut<T>(this.apiUrl + path, <T>this.preprocessData(body), configuredOptions ?? options).pipe(catchError(err => this.catchHttpError<T>(err, path, body)));
    }
