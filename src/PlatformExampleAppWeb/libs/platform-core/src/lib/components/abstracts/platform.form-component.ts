 ...
 */
@Directive()
export abstract class PlatformFormComponent<TViewModel extends IPlatformVm> extends PlatformVmComponent<TViewModel> implements IPlatformFormComponent<TViewModel>, OnInit {
    ...
    public isFormLoading = computed(() => (this.formStatus$() == 'PENDING' && this.form?.status == 'PENDING') || this.status$() == ComponentStateStatus.Loading);
        ...
        keys(vmFormValues).forEach(formKey => {
            ...
            if (isDifferent(vmFormKeyValue, currentReactiveFormValues[formKey], false, true)) {
                if (formControl instanceof FormArray && vmFormKeyValue instanceof Array) {
                    const listControlformConfig = <PlatformFormGroupControlConfigPropArray<ArrayElement<TViewModel[keyof TViewModel]>>>this.formConfig.controls[vmFormKey];
                    ...
                    vmValueItem === undefined
                        ...
                        : (<unknown[]>vmValueItem).map((vmValueArrayItem, index) => {
                              const currentReactiveFormValueArrayItem =
                                  <unknown[]>currentReactiveFormValues[formControlKey] != null ? (<unknown[]>currentReactiveFormValues[formControlKey])[index] : null;

                              if (!isSinglePrimitiveOrImmutableType(vmValueArrayItem) && !isSinglePrimitiveOrImmutableType(currentReactiveFormValueArrayItem)) {
                                  return this.getFromVmFormValues(<Dictionary<unknown>>vmValueArrayItem, <Dictionary<unknown>>currentReactiveFormValueArrayItem);
                              }
    ...
    public getFormControls<TFormModel, TKey extends keyof TFormModel>(formGroup: FormGroup<PlatformFormGroupControls<TFormModel>>, key: TKey): FormControl<TFormModel[TKey]> {
     ...
     */
    protected formControlArrayFor<TItemModel>(items: TItemModel[], formItemControl: (item: TItemModel) => FormControl<TItemModel>): FormArray<FormControl<TItemModel>> {
    ...
    protected buildFromArrayControlItem<TFormModel, TItemModel extends ArrayElement<TFormModel[keyof TFormModel]>>(
        ...
        function buildFromArrayControlItemValueChangesSubscriptionKey(formArrayControlKey: keyof TFormModel, formControlKey: string, modelItemIndex: number): string {
