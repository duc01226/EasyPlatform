...
export abstract class PlatformComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
                            ...
                            if (this.isStateLoading()) {
                                this.devModeCheckLoadingStateElementTimeoutId = setTimeout(() => {
                                    ...
                                    const devModeCheckLoadingStateElements =
                                        typeof this.devModeCheckLoadingStateElement == 'string' ? [this.devModeCheckLoadingStateElement] : this.devModeCheckLoadingStateElement;
                                    const findInRootElement = this.devModeCheckLoadingOrErrorAllowInGlobalDocumentBody ? document : this.elementRef.nativeElement;
                            ...
                            if (this.errorMsg$() != null) {
                                setTimeout(() => {
                                    ...
                                    const devModeCheckErrorStateElements =
                                        typeof this.devModeCheckErrorStateElement == 'string' ? [this.devModeCheckErrorStateElement] : this.devModeCheckErrorStateElement;
                                    const findInRootElement = this.devModeCheckLoadingOrErrorAllowInGlobalDocumentBody ? document : this.elementRef.nativeElement;
     ...
     *   );
     */
    public observerLoadingErrorState<T>(requestKey?: string | null, options?: PlatformObserverLoadingErrorStateOptions): (source: Observable<T>) => Observable<T> {
