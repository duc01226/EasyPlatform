...
import { filter, clone as lodashClone, cloneDeep as lodashCloneDeep, keys as lodashKeys, values as lodashValues, union } from 'lodash-es';
...
export function keys<T extends object>(source: T, ignorePrivate: boolean = true, excludeKeys?: (keyof T)[], includeFunction?: boolean): (keyof T & string)[] {
    ...
    for (const key in source) {
        if ((typeof (<any>source)[key] != 'function' || includeFunction == true) && (ignorePrivate == false || !key.startsWith('_')) && !excludeKeys?.includes(key)) {
            if (key.startsWith('_')) {
...
export function toPlainObj<T>(source: T, ignorePrivate: boolean = true, onlyKeysExistInPartialObject?: object): any {
    ...
    keys(source, ignorePrivate).forEach(key => {
        if (onlyKeysExistInPartialObject == undefined || (<any>onlyKeysExistInPartialObject)[key] !== undefined) objResult[key] = toPlainObj((<any>source)[key], ignorePrivate);
    });
    return objResult;
}
...
export function immutableUpdate<TObject extends object>(
    ...
    if (typeof partialStateOrUpdaterFn == 'function') {
        ...
        const clonedLocalDevToCheckStateMutationJson =
            options?.updaterNotDeepMutate == true && PLATFORM_CORE_GLOBAL_ENV.isLocalDev ? JSON.stringify(toPlainObj(clonedLocalDevToCheckStateMutation)) : undefined;
...
function assignOrSetDeep<T extends object>(
    ...
    if (target instanceof Array && source instanceof Array) {
        ...
        keys(source, undefined, undefined, includeFunction).forEach(key => {
            ...
            if ((targetKeyPropertyDescriptor?.get != null && targetKeyPropertyDescriptor?.set == null) || targetKeyPropertyDescriptor?.writable == false) return;
    ...
    function setNewValueToTargetKeyProp(key: keyof T & string, currentDeepLevel: number) {
        ...
        if (checkTwoValueShouldSetDirectlyAndNotSetDeep((<any>target)[key], (<any>source)[key]) == false && (maxDeepLevel == undefined || currentDeepLevel + 1 <= maxDeepLevel)) {
            ...
            if ((<any>target)[key] instanceof Array && (<any>source)[key] instanceof Array) {
                assignOrSetDeepArray(newValueToSetToTarget, (<any>source)[key], cloneSource, makeTargetValuesSameSourceValues, checkDiff, maxDeepLevel, currentDeepLevel + 1);
            } else {
                assignOrSetDeep(newValueToSetToTarget, (<any>source)[key], cloneSource, makeTargetValuesSameSourceValues, checkDiff, maxDeepLevel, currentDeepLevel + 1);
            }
        }
        ...
        for (let i = 0; i < sourceArray.length; i++) {
            ...
            if (checkTwoValueShouldSetDirectlyAndNotSetDeep(targetArray[i], sourceArray[i]) || (maxDeepLevel != null && currentDeepLevel + 1 > maxDeepLevel)) {
                ...
                targetArray[i] = clone(targetArray[i], clonedTargetArrayItem => {
                    assignOrSetDeep(clonedTargetArrayItem, sourceArray[i], cloneSource, makeTargetValuesSameSourceValues, checkDiff, maxDeepLevel, currentDeepLevel + 1);
                });
            }
        }
...
export function pipe<T, TResult1, TResult2, TResult3>(input: T, fns: [(arg: T) => TResult1, (arg: TResult1) => TResult2, (arg: TResult2) => TResult3]): TResult3;
...
export function pipe<T, TResult1, TResult2, TResult3, TResult4, TResult5, TResult6>(
    input: T,
    fns: [(arg: T) => TResult1, (arg: TResult1) => TResult2, (arg: TResult2) => TResult3, (arg: TResult3) => TResult4, (arg: TResult4) => TResult5, (arg: TResult5) => TResult6]
): TResult6;
