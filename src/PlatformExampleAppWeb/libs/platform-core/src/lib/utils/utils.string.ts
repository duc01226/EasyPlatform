...
export function string_stringUrlToQueryDic(url: string, urlOrigin?: string) {
    ...
    const onlyQueryUrl = hashIndex < querySperatorIndex ? urlWithoutOrigin.substr(querySperatorIndex + 1) : urlWithoutOrigin.substring(querySperatorIndex + 1, hashIndex);
...
export function string_transformSpaceTo(text: string, suffix: string | undefined = undefined, linkingSymbol: string = '_') {
    return text.replace(/ /g, `${linkingSymbol}`).toLowerCase() + (suffix != undefined ? `${linkingSymbol}${suffix.toLowerCase()}` : '');
}
