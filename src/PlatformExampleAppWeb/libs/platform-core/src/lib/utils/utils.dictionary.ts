...
export function dictionary_upsert<T>(
    ...
    function removeNotExistedItemsInNewData(state: Dictionary<Partial<T>>, newDataDic: Dictionary<Partial<T>>): void {
        const removeItemIds = lodashKeys(state).filter(id => newDataDic[id] == null && (removeNotExistedItemsFilter == null || removeNotExistedItemsFilter(state[id]!)));
    ...
    function modifyDict(state: Dictionary<T>, modifyDicAction: (state: Dictionary<T>) => void | Dictionary<T>): Dictionary<T> {
