...
import { PlatformCoreModuleConfig, PlatformEventManager, PlatformPagedResultDto, PlatformRepository } from '@libs/platform-core';
    ...
    public search(query: SearchTextSnippetQuery): Observable<PlatformPagedResultDto<TextSnippetDataModel>> {
        return this.processUpsertData({
            ...
            finalResultBuilder: (repoData, apiResult) => {
                apiResult.items = <TextSnippetDataModel[]>apiResult.items.map(item => repoData[<string>item.id]).filter(_ => _ != null);
                return apiResult;
            },
