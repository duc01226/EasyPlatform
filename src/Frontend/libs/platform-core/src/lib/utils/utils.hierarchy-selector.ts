    ...
    return items.reduce(
        (accumulator, item) => {
            if (string_isIncluded((<any>item)[labelProp], searchValue)) {
                ...
                const newItems = hierarchy_searchNodeParentByLabelProp((<any>item)[childrenProp], searchValue, valueProp, labelProp, parentProp, childrenProp);
...
export function findItemByLabelAndValueProp<T>(tree: T[], labelProp: keyof T, value: string, childNodeProp: keyof T): T | undefined {
...
export function findRootItemByChildLabelAndValueProp<T>(tree: T[], labelProp: keyof T, value: string, childNodeProp: keyof T): T | undefined {
    let result: T | undefined;

    tree.some((node: any) => {
        if ((<any>node)[childNodeProp]) {
            const childResult = findItemByLabelAndValueProp((<any>node)[childNodeProp], labelProp, value, childNodeProp);
            if (childResult) {
