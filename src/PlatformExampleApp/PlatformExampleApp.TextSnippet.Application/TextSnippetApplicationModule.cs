...
public class TextSnippetApplicationModule : PlatformApplicationModule
{
    ...
    protected override PlatformApplicationSettingContext DefaultApplicationSettingContextFactory(IServiceProvider serviceProvider)
    {
        return new PlatformApplicationSettingContext(serviceProvider)
        {
            ...
            IsDebugInformationMode =
                serviceProvider.GetRequiredService<IConfiguration>().GetValue<bool?>(PlatformApplicationSettingContext.DefaultIsDebugInformationModeConfigurationKey) == true,
        };
    }
    ...
    {
        return await provider.ExecuteInjectScopedAsync<UserEntity?>(
            async (ITextSnippetRootRepository<UserEntity> userRepository, IPlatformCacheRepositoryProvider cacheRepositoryProvider) =>
            {
