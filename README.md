...
```csharp
var validationResult = await this
    .ValidateRequestAsync(request)
    .AndAsync(async () => await ValidateCustomerExistsAsync(request.CustomerId, cancellationToken))
    .AndAsync(async () => await ValidateInventoryAsync(request.Items, cancellationToken));
```
...
export class ProductListStore extends PlatformVmStore<ProductListViewModel> {
    ...
    // Remember user preferences and filters
    protected cachedUIState = {
...
export class UserManagementStore extends PlatformVmStore<UserManagementViewModel> {
    ...
    public saveUser() {
        ...
        if (user.age < 18 && user.role === 'admin') {
            this.setError('Minors cannot be administrators');
            return;
        }

        // API call with caching strategy
        this.setLoading(true);
        this.userApi.saveUser(user).pipe(this.handleApiResponse('saveUser')).subscribe();
    }
...
export class UserListStore extends PlatformVmStore<UserListViewModel> {
    ...
    // Step 1: Return cached data instantly (no loading screen)
    public initOrReloadVm = (isReload: boolean) => {
        const cachedUsers = this.getCachedData();

        if (cachedUsers && !isReload) {
            // Instant display of cached data
            this.updateVm({ users: cachedUsers });

            // Background refresh (small progress indicator)
            this.refreshInBackground();
            return cachedUsers;
        }

        // First time load or explicit refresh
        return this.loadFromApi();
    };

    private refreshInBackground() {
        this.setReloading(true); // Shows small progress bar, not full loading

        this.userApi
            .getUsers()
            .pipe(this.handleApiResponse('backgroundRefresh'))
            .subscribe(freshData => {
                this.updateVm({ users: freshData });
                this.setCachedData(freshData); // Update cache
...
export class UserApiService extends PlatformApiService {
    getUsers(): Observable<User[]> {
        return this.get<User[]>('/api/users').pipe(
            // Automatic caching with 5-minute expiry
            this.withCaching('users-list', { expiry: 300000 }),

            // Cache-then-refresh: return cache immediately, refresh in background
            this.withCacheThenRefresh()
        );
    }
}
```
...
@Component({
    template: `
        <form [formGroup]="formGroup">
            <!-- Two-way binding with view model -->
            <input formControlName="firstName" [(ngModel)]="vm().user.firstName" [platformFormControl]="'firstName'" />

            <input formControlName="email" [(ngModel)]="vm().user.email" [platformFormControl]="'email'" />
        </form>
    `
})
export class UserFormComponent extends PlatformFormComponent<UserViewModel> {
    // Automatic form setup with view model binding
    protected setupFormControls() {
        return {
            firstName: [this.vm().user.firstName, [Validators.required]],
            email: [this.vm().user.email, [Validators.email]]
        };
    }

    // Automatic sync: Form changes update view model
    protected onFormValueChanges(formValue: any) {
        this.updateVm({
            user: { ...this.vm().user, ...formValue }
        });
    }

    // Automatic sync: View model changes update form
    protected onVmChanged() {
        this.patchFormValue(this.vm().user);
    }
}
```
...
```typescript
export class DynamicFormComponent extends PlatformFormComponent<FormViewModel> {
    // Conditional validation based on view model state
    protected getConditionalValidators() {
        return {
            salary: () => {
                if (this.vm().user.role === 'manager') {
                    return [Validators.required, Validators.min(50000)];
                }
                return [Validators.min(30000)];
            }
        };
    }

    // Real-time form state in view model
    protected onFormStateChanged(state: FormState) {
        this.updateVm({
            formState: {
                isValid: state.valid,
                isDirty: state.dirty,
